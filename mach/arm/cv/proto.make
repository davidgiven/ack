...
cv.$(SUF):	$(SRC_DIR)/cv.c $(SRC_DIR)/arm.h
	$(CC) $(COPTIONS) -I$(TARGET_HOME)/h -c $(SRC_DIR)/cv.c

install:	all
	-mkdir $(TARGET_HOME)/lib.bin/arm
	cp cv $(TARGET_HOME)/lib.bin/arm/cv

cmp:	all
	-cmp cv $(TARGET_HOME)/lib.bin/arm/cv
...
pr:
	@pr $(SRC_DIR)/proto.make $(SRC_DIR)/arm.h $(SRC_DIR)/cv.c
