...
{
	tracef(k, "%%%d(t%x)", vreg->id, vreg->type);
	if (vreg->congruence)
		tracef(k, "(g%d)", vreg->congruence->id);
}
	...
	for (i = 0; i < dominance.preorder.count; i++)
	{
		...
		if (bb->phis.count > 0)
		{
			...
			for (j = 0; j < bb->phis.count; j++)
			{
				...
				tracef(
				    k, " %%%d(via %s)=>", phi->ir->result->id, phi->prev->name);
				print_vreg(k, vreg);
			}
			tracef(k, "\n");
		}
