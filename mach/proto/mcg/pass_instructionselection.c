...
static void append_copy(struct vreg* oldvreg, struct vreg* newvreg)
{
	struct hop* hop = new_hop(current_bb, NULL);
	hop->output = newvreg;
	array_append(&hop->ins, oldvreg);
	array_append(&hop->outs, newvreg);
	array_append(&oldvreg->used, hop);
	array_append(&newvreg->used, hop);
	hop->is_copy = true;
...
static void connect_input_reg(int child, regclass_t regclass)
{
	struct vreg* tvreg = find_vreg_of_child(child);
	struct constraint* c;
	struct vreg* vreg = new_vreg();

	vreg->regclass = regclass;
	append_copy(tvreg, vreg);

	tracef('I', "I: connecting child %d to %%%d\n", child, vreg->id);
	imap_put(&current_hop->inputmap, child, vreg);
...
static void constrain_input_reg_preserved(int child)
{
	...
	if (!vreg)
		fatal(
		    "child %d of instruction is not a register and cannot be "
		    "constrained",
		    child);
...
static void connect_output_reg(regclass_t regclass)
{
...
static struct insn*
walk_instructions(struct burm_node* node, int goal, bool top)
{
	...
	if (goal)
	{
		...
		tracef(
		    ...
		    insn->insndata->is_fragment ? "fragment" : "instruction", insn_no,
		    insn->insndata->name);
		...
		if (!insn->insndata->is_fragment)
		{
			...
			assert(!current_hop->toutput);
			if (current_hop->output)
			{
				tracef(
				    'I', "I: generating output copy for %%%d\n",
				    current_hop->output->id);
				current_hop->toutput = new_vreg();
				current_hop->toutput->regclass
				    = find_generic_class_for(current_hop->output->regclass);
				current_hop->toutput->in_transit = true;
				append_copy(current_hop->output, current_hop->toutput);
			}

			if ((goal != burm_stmt_NT) && !insn->ir->result)
				insn->ir->result = insn->hop->toutput ? insn->hop->toutput
				                                      : insn->hop->output;
		}
	}
		...
		if (current_ir->opcode == IR_PHI)
		{
			...
			for (j = 0; j < current_ir->u.phivalue.count; j++)
			{
				struct basicblock* parentbb
				    = current_ir->u.phivalue.item[j].left;
