...
static void count_usage(proc_p p, item_p item, short nrloops, short sloopcnt[], short dloopcnt[])
{
	...
	for (ui = Lfirst(item->it_usage); ui != (Lindex)0; ui = Lnext(ui, item->it_usage))
	{
		u = (time_p)Lelem(ui);
		...
		for (li = Lfirst(loops); li != (Lindex)0; li = Lnext(li, loops))
		{
			l = (loop_p)Lelem(li);
			sloopcnt[l->lp_id]++;
			dloopcnt[l->lp_id] += (IS_FIRM(u->t_bblock) ? loop_scale(lev) : 1);
		}
	}
}

static alloc_p cons_alloc(
    item_p item,
    interv_p timespan,
    short stat_usecount,
    short dyn_usecount,
...
static bblock_p init_point(item_p item)
{
	...
	for (ti = Lfirst(item->it_usage); ti != (Lindex)0; ti = Lnext(ti, item->it_usage))
	{
		t = (time_p)Lelem(ti);
		b = t->t_bblock;
...
static void add_blocks(bblock_p b, cset* s, interv_p* span)
{
	...
	if (!Cis_elem(b->b_id, *s))
	{
		...
		for (pi = Lfirst(b->b_pred); pi != (Lindex)0; pi = Lnext(pi, b->b_pred))
		{
    ...
    alloc_p* alloc_list_p)
{
	...
	for (li = Lfirst(loops); li != (Lindex)0; li = Lnext(li, loops))
	{
		...
		if (sloopcnt[lp->lp_id] != 0 && !updates_needed(lp, item)
		    && !((header = lp->LP_HEADER) == (bblock_p)0 && MUST_INIT(item, lp->lp_entry)))
		{
...
alloc_p build_alloc_list(proc_p p, short nrloops, item_p itemlist)
{
	...
	for (item = itemlist; item != (item_p)0; item = item->it_next)
	{
		...
		allocs_of_item(p, item, p->p_loops, sloopcnt, dloopcnt, &alloc_list);
	}
...
void build_rivals_graph(alloc_p alloclist)
{
	...
	for (alloc = alloclist; alloc != (alloc_p)0; alloc = alloc->al_next)
	{
		...
		{
			if (x->al_item != alloc->al_item && not_disjoint(alloc->al_busy, x->al_busy))
			{
