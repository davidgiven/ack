...
static bool same_instr(line_p l1, line_p l2)
{
	...
	switch (TYPE(l1))
	{
		...
		case OPOFFSET:
			return OFFSET(l1) == OFFSET(l2);
		case OPPROC:
			return PROC(l1) == PROC(l2);
		case OPOBJECT:
...
static void jump_cross(line_p l1, line_p l2, bblock_p b1, bblock_p b2)
{
	...
	b->b_succ = b1->b_succ;
	/* SUCC(b1) = {b} */
	b1->b_succ = Lempty_set();
	Ladd(b, &b1->b_succ);
	/* SUCC(b2) = {b} */
	Ldeleteset(b2->b_succ);
	b2->b_succ = Lempty_set();
	Ladd(b, &b2->b_succ);
	/* PRED(b) = {b1,b2} */
	b->b_pred = Lempty_set();
	Ladd(b1, &b->b_pred);
...
static void showinstr(line_p lnp)
{
	...
	switch (TYPE(lnp))
	{
		...
		case OPSHORT:
			...
			break;
		case OPOBJECT:
			printf(" %d", OBJ(lnp)->o_id);
			break;
		case OPOFFSET:
			printf(" %ld", OFFSET(lnp));
			break;
		default:
			printf(" ?");
			break;
