...
static void outdocc(obj_p obj)
{
	...
	{
		dbl->d_flags2 |= DF_SYMOUT;
		if (dnames[dbl->d_id] != 0 && (dbl->d_flags1 & DF_EXTERNAL) == 0)
		{
			outinst(ps_ina);
			outdsym(dbl);
		}
...
static void cputargs(line_p lnp)
{
	...
	while (ap != (arg_p)0)
	{
		...
		if (cnt++ > 10 && ap != (arg_p)0 && (INSTR(lnp) == ps_con || INSTR(lnp) == ps_rom))
		{
			cnt = 0;
...
static void outoperand(line_p lnp)
{
	...
	{
		case OPNO:
			if (INSTR(lnp) <= sp_lmnem && (em_flag[INSTR(lnp) - sp_fmnem] & EM_PAR) != PAR_NO)
			{
				outbyte((byte)sp_cend);
			}
			break;
