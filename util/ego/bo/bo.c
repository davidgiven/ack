...
static line_p last_code(line_p lines, bool skip_pseu)
{
	...
	if (skip_pseu)
	{
		while (l && (INSTR(l) < sp_fmnem || INSTR(l) > sp_lmnem))
			l = PREV(l);
	}
	return l;
}
...
static void bo_optloop(proc_p p, bblock_p b, bblock_p x, line_p bra, line_p bcc)
{
	...
	for (prevb = p->p_start; prevb != (bblock_p)0 && prevb->b_next != x; prevb = prevb->b_next)
		;
	if (prevb == (bblock_p)0)
	{
		p->p_start = x->b_next;
...
static void bo_tryloop(proc_p p, lset loop)
{
	...
	for (i = Lfirst(loop); i != (Lindex)0; i = Lnext(i, loop))
	{
		...
		{
			j = Lfirst(b->b_succ);
			if (j != (Lindex)0 && (bra = last_instr(b)) != (line_p)0 && INSTR(bra) == op_bra)
			{
				x = (bblock_p)Lelem(j); /* single successor */
				if (Lis_elem(b->b_next, x->b_succ) && is_bcc((bcc = last_instr(x))))
				{
