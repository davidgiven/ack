	...
	if (p && p->t_idf)
		switch (p->t_idf->id_reserved)
		{
			...
			case LIST:
				...
				fputs(
				    "  line <start> or some lines before the first statement of <func> or\n",
				    db_out);
			...
			case FRAME:
				...
				fputs(
				    "  procedure has frame 0. If <num> is not given, print the 'current' frame.\n",
				    db_out);
				fputs(
				    "  If <num> is given with a + or -, go up or down <num> frames relative\n",
				    db_out);
			...
			case RUN:
				...
				fputs(
				    "  Start executing debuggee with command line arguments <args> and\n", db_out);
			...
			case STOP:
				...
				fputs(
				    "  becomes true.  If no <cond> is given, stop when <pos> is reached.\n",
				    db_out);
			...
			case WHEN:
				...
				fputs(
				    "  Execute the <command>s when position <pos> is reached, and then when\n",
				    db_out);
				fputs("  <cond> becomes true. If no <pos> is given, do this when <cond>\n", db_out);
				fputs(
				    "  becomes true.  If no <cond> is given, do this when <pos> is reached.\n",
				    db_out);
				fputs("  Either a position or a condition (or both) must be given.\n", db_out);
				return;
			case CONT:
			...
			case WHERE:
				...
				fputs("w [ <cnt> ]\n", db_out);
				fputs(
				    "  List all, or the top <cnt> or the bottom -<cnt> active functions.\n",
				    db_out);
				return;
			case STATUS:
				fputs("status\n", db_out);
			...
			case DELETE:
				...
				fputs("d [ <num> [ , <num> ] ...] \n", db_out);
				fputs(
				    "  Remove the command(s) corresponding to <num> (as displayed by 'status').\n",
				    db_out);
			...
			case PRINT:
				...
				fputs("p [ <exp> [ , <exp> ] ...]\n", db_out);
				fputs(
				    "  Print the value of each <exp>, or repeat the last print command\n", db_out);
				return;
			case DISPLAY:
			...
			case DUMP:
				fputs("dump\n", db_out);
				fputs(
				    "  Saves the state of the debuggee; it can be restored with the restore "
				    "command.\n",
				    db_out);
				return;
			case RESTORE:
			...
			case TRACE:
				...
				fputs(
				    "  If it indicates a line number, only display tracing information\n", db_out);
				fputs("  whenever the source line is reached.\n", db_out);
				fputs(
				    "  If <cond> is given, only display tracing info when it evaluates to "
				    "non-zero.\n",
				    db_out);
				return;
			case FIND:
				fputs("find <name>\n", db_out);
				fputs(
				    "  Prints the fully qualified name of all symbols matching <name>.\n", db_out);
				return;
			case WHICH:
				fputs("which <name>\n", db_out);
			...
			case DISABLE:
				...
				fputs(
				    "  Disable the command(s) corresponding to <num> (as displayed by 'status').\n",
				    db_out);
			...
			case ENABLE:
				...
				fputs(
				    "  Enable the command(s) corresponding to <num> (as displayed by 'status'.)\n",
				    db_out);
				fputs(
				    "  If no <num> is given, enable the current stopping point (not effective).\n",
				    db_out);
				return;
		}
	else if (p && p->t_str)
...
void do_continue(p_tree p)
{
	...
	if (p)
	{
		...
		if (p->t_args[1])
		{
			...
			{
				error("cannot continue at line %d", p->t_args[1]->t_lino);
				return;
			}
			if (!set_pc(a))
			{
...
void do_regs(p_tree p)
{
	...
	fprintf(
	    db_out, "\tProgramCounter=\t0x%lx\n\tHeapPointer = \t0x%lx\n", (long)buf[2], (long)buf[3]);
...
void do_print(p_tree p)
{
	...
	switch (p->t_oper)
	{
		...
		case OP_LINK:
			...
			break;
		default:
			if (interrupted || !eval_expr(p, &buf, &size, &tp))
				return;
...
void do_set(p_tree p)
{
	...
	if (interrupted || !eval_desig(p->t_args[0], &a, &size, &tp)
	    || !eval_expr(p->t_args[1], &buf, &size2, &tp2) || !convert(&buf, &size2, &tp2, tp, size))
	{
		if (buf)
			free(buf);
		return;
