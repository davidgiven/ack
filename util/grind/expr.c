...
*/
static t_addr get_addr(sym, psize)
p_symbol sym;
long* psize;
{
		...
		case LOCVAR:
			...
			*/
			...
			if (sym->sy_class == LOCVAR)
			{
				...
				return EM_regs[sym->sy_name.nm_value < 0 ? 0 : 1] + (t_addr)sym->sy_name.nm_value;
			}
...
/* static int	get_value(p_symbol sym; char **pbuf; long *psize);
   ...
   used to determine from which stack frame the search must start.
*/
static int get_value(sym, pbuf, psize)
p_symbol sym;
char** pbuf;
long* psize;
{
...
long get_int(buf, size, class) char* buf;
long size;
int class;
{
	long l;
	...
	switch ((int)size)
	{
		case sizeof(char):
			l = *buf;
			if (class == T_INTEGER && l >= 0x7F)
				l -= 256;
			else if (class != T_INTEGER && l < 0)
				l += 256;
			break;
		case sizeof(short):
			l = *((short*)buf);
			if (class == T_INTEGER && l >= 0x7FFF)
				l -= 65536;
			else if (class != T_INTEGER && l < 0)
				l += 65536;
			break;
...
put_int(buf, size, value) char* buf;
long size;
long value;
{
	switch ((int)size)
...
put_real(buf, size, value) char* buf;
long size;
double value;
{
	switch ((int)size)
...
int convert(pbuf, psize, ptp, tp, size)
char** pbuf;
long* psize;
p_type* ptp;
p_type tp;
		...
		switch ((*ptp)->ty_class)
		{
			...
			case T_ENUM:
				...
				switch (tp->ty_class)
				{
					...
					case T_REAL:
						put_real(
						    *pbuf, size,
						    (*ptp)->ty_class == T_INTEGER ? (double)l : (double)(unsigned long)l);
						*psize = size;
						*ptp = tp;
						return 1;
					default:
			...
			case T_REAL:
				...
				switch (tp->ty_class)
				{
					...
					case T_POINTER:
						if (tp == bool_type)
							put_int(*pbuf, size, (long)(d != 0));
						else
							put_int(*pbuf, size, (long)d);
						*psize = size;
						*ptp = tp;
						return 1;
					case T_REAL:
...
static int not_op(p, pbuf, psize, ptp)
p_tree p;
char** pbuf;
long* psize;
p_type* ptp;
...
static int bnot_op(p, pbuf, psize, ptp)
p_tree p;
char** pbuf;
long* psize;
p_type* ptp;
...
static int ptr_addr(p, paddr, psize, ptp)
p_tree p;
t_addr* paddr;
long* psize;
p_type* ptp;
...
static int deref_op(p, pbuf, psize, ptp)
p_tree p;
char** pbuf;
long* psize;
p_type* ptp;
...
static int addr_op(p, pbuf, psize, ptp)
p_tree p;
char** pbuf;
long* psize;
p_type* ptp;
...
static int unmin_op(p, pbuf, psize, ptp)
p_tree p;
char** pbuf;
long* psize;
p_type* ptp;
...
static int unplus_op(p, pbuf, psize, ptp)
p_tree p;
char** pbuf;
long* psize;
p_type* ptp;
...
static p_type balance(tp1, tp2)
p_type tp1, tp2;
{

	if (tp1->ty_class == T_SUBRANGE)
		tp1 = tp1->ty_base;
	if (tp2->ty_class == T_SUBRANGE)
		tp2 = tp2->ty_base;
	if (tp1 == tp2)
		return tp2;
...
static int andor_op(p, pbuf, psize, ptp)
p_tree p;
char** pbuf;
long* psize;
p_type* ptp;
{
	    ...
	    && convert(&buf, &size, &tp, target_tp, target_tp->ty_size))
	{
		...
		put_int(*pbuf, *psize, p->t_whichoper == E_AND ? (long)(l1 && l2) : (long)(l1 || l2));
		free(buf);
		return 1;
	}
	if (buf)
...
static int arith_op(p, pbuf, psize, ptp)
p_tree p;
char** pbuf;
long* psize;
p_type* ptp;
{
	...
	if (!(eval_expr(p->t_args[0], pbuf, psize, ptp) && eval_expr(p->t_args[1], &buf, &size, &tp)))
	{
		return 0;
	}
	if ((*ptp)->ty_class == T_POINTER)
	{
		if (currlang != c_dep || (p->t_whichoper != E_PLUS && p->t_whichoper != E_MIN))
		{
			error("illegal operand type(s)");
			free(buf);
			return 0;
				...
				switch (p->t_whichoper)
				{
					...
					case E_ZDIV:
						...
						if (balance_tp->ty_class == T_INTEGER)
						{
							...
							l1 = (unsigned long)l1 / (unsigned long)l2;
						break;
					case E_MOD:
					case E_ZMOD:
						if (!l2)
...
static int sft_op(p, pbuf, psize, ptp)
p_tree p;
char** pbuf;
long* psize;
p_type* ptp;
			...
			case T_UNSIGNED:
				...
				switch (p->t_whichoper)
				{
					case E_LSFT:
						...
						break;
					case E_RSFT:
						if (tp->ty_class == T_INTEGER)
							l1 >>= (int)l2;
						else
							l1 = (unsigned long)l1 >> (int)l2;
						break;
				}
				break;
			default:
...
static int cmp_op(p, pbuf, psize, ptp)
p_tree p;
char** pbuf;
long* psize;
p_type* ptp;
...
static int in_op(p, pbuf, psize, ptp)
p_tree p;
char** pbuf;
long* psize;
p_type* ptp;
...
static int array_addr(p, paddr, psize, ptp)
p_tree p;
t_addr* paddr;
long* psize;
p_type* ptp;
{
	...
	if (eval_desig(p->t_args[0], paddr, psize, ptp) && eval_expr(p->t_args[1], &buf, &size, &tp))
	{
...
static int array_op(p, pbuf, psize, ptp)
p_tree p;
char** pbuf;
long* psize;
p_type* ptp;
...
static int select_addr(p, paddr, psize, ptp)
p_tree p;
t_addr* paddr;
long* psize;
p_type* ptp;
{
	...
	if (eval_desig(p->t_args[0], paddr, psize, ptp))
	{
		...
		{
			if (!strcmp(f->fld_name, p->t_args[1]->t_str))
				break;
		}
		if (!nf)
		{
...
static int select_op(p, pbuf, psize, ptp)
p_tree p;
char** pbuf;
long* psize;
p_type* ptp;
...
static int derselect_op(p, pbuf, psize, ptp)
p_tree p;
char** pbuf;
long* psize;
p_type* ptp;
...
static int (*bin_op[])()
    = { 0,      0,        0,         andor_op, andor_op, arith_op, arith_op, arith_op,     arith_op,
	    in_op,  array_op, arith_op,  arith_op, arith_op, cmp_op,   cmp_op,   cmp_op,       cmp_op,
	    cmp_op, cmp_op,   select_op, arith_op, arith_op, arith_op, 0,        derselect_op, sft_op,
	    sft_op, 0 };

int eval_expr(p, pbuf, psize, ptp)
p_tree p;
char** pbuf;
long* psize;
p_type* ptp;
...
int eval_desig(p, paddr, psize, ptp)
p_tree p;
t_addr* paddr;
long* psize;
p_type* ptp;
