...
void init_run(void)
{
	...
	for (i = 3; i <= 6; i++)
		close(i);
	if (pipe(fild1) < 0 || pipe(fild2) < 0 || fild1[0] != 3 || fild2[1] != 6)
	{
	...
	child_pid = 0;
	if (currfile)
		CurrentScope = currfile->sy_file->f_scope;
	currline = 0;
}
...
void start_child(p_tree p)
{
	...
	if (child_pid == 0)
	{
		...
		if (in_redirect)
		{
			...
			close(0);
			if ((fd = open(in_redirect, 0)) < 0 || (fd != 0 && dup2(fd, 0) < 0))
			{
				perror(progname);
				exit(1);
			}
		...
		if (out_redirect)
		{
			...
			close(1);
			if ((fd = creat(out_redirect, 0666)) < 0 || (fd != 1 && dup2(fd, 1) < 0))
			{
				perror(progname);
				exit(1);
			}
	...
	*/
	...
	if (!restoring)
	{
		...
		if (!stop_reason)
			...
			(void)stopped("stopped", curr_stop, stop_reason);
	}
}
...
static int could_send(struct message_hdr* m, int stop_message)
{
	...
	for (;;)
	{
		...
		if (child_dead)
		{
			...
			{
				fprintf(db_out, "child died with signal %d\n", child_status & 0177);
			}
			else
			{
				fprintf(db_out, "child terminated, exit status %d\n", child_status >> 8);
			}
			init_run();
			level--;
			return 1;
		...
		if (type == M_END_SS)
			type = 0;
		else if (type == M_OK || type == M_DB_SS)
			type = 1;
		else
			type = 2;
		if (m->m_type & M_DB_RUN)
...
t_addr get_dump(char** globbuf, char** stackbuf)
{
	...
	*globbuf = malloc((unsigned)(sz + sizeof(struct message_hdr)));
	if (!*globbuf || !ureceive(*globbuf + sizeof(struct message_hdr), sz) || !ugetm(&m))
	{
		if (*globbuf)
			free(*globbuf);
		else
			error("could not allocate enougn memory");
		return 0;
	}
...
int put_dump(char* globbuf, char* stackbuf)
{
	...
	return uputm(globm) && usend(globbuf, BUFTOI(globm->m_buf + 1, LS)) && uputm(stackm)
	    && usend(stackbuf, BUFTOI(stackm->m_buf + 1, LS)) && ugetm(&m)
