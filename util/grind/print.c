...
char* format)
{
	...
	if (!i)
	{
		fprintf(db_out, compressed ? "?%ld?" : "unknown enumeration value %ld", v);
	}
}
	...
	while (i--)
	{
		...
		if (par->par_kind == 'v' || par->par_kind == 'i')
		{
			...
			*/
			...
			print_val(par->par_type, par->par_type->ty_size, p, 1, 0, (char*)0);
		if (i)
			fputs(", ", db_out);
		par++;
	}
	...
	switch (tp->ty_class)
	{
		...
		case T_ARRAY:
			if ((!format || strchr(format, 'a') == 0)
			    && (tp->ty_elements == char_type || tp->ty_elements == uchar_type))
			{
			...
			{
				fprintf(
				    db_out, "\n%*c%s%*c", indent, ' ', currlang->open_array_display,
			...
			{
				print_val(
				    tp->ty_elements, tp->ty_elements->ty_size, addr, compressed, indent, format);
				addr += elsize;
				if (compressed && i > 1)
				{
					fprintf(db_out, ", ...");
		...
		case T_STRUCT:
		{
			...
			for (i = tp->ty_nfields; i; i--, fld++)
			{
				...
				if (fld->fld_bitsize < (sz << 3))
				{
					...
					print_val(
					    fld->fld_type, sz, addr + (fld->fld_pos >> 3), compressed, indent, format);
				if (compressed && i > 1)
				{
					fprintf(db_out, ", ...");
					break;
		...
		case T_SET:
		{
			...
			{
				fprintf(
				    db_out, "\n%*c%s%*c", indent, ' ', currlang->open_set_display,
