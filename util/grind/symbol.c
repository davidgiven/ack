...
p_symbol add_file(char* s)
{
	...
	if (p)
	{
		...
		*p = 0;
		s = strdup(s);
		*p = c;
		sym1 = NewSymbol(s, PervasiveScope, FILELINK, (struct outname*)0);
...
int consistent(p_tree p, p_scope sc)
{
	...
	switch (p->t_oper)
	{
		case OP_NAME:
#define CLASS                                                                                      \
	(FILELINK | FILESYM | PROC | FUNCTION | MODULE | TYPE | VAR | REGVAR | LOCVAR | VARPAR         \
	 | LBOUND | UBOUND)
...
p_symbol identify(p_tree p, int class_set)
{
	...
	switch (p->t_oper)
	{
		...
		case OP_SELECT:
			...
			{
				int temp;
				if ((s->sy_class & class_set) && (temp = consistent(p, s->sy_scope)))
				{
					if (temp > precise)
					{
						sym = s;
...
static void pr_scopes(p_scope sc)
{
	...
	if (sc->sc_definedby)
	{
		if (sc->sc_definedby->sy_class == FILESYM && sc->sc_definedby->sy_file->f_base)
		{
...
void resolve_cross(p_type tp)
{
	...
	while (sym)
	{
		if (sym->sy_class == TAG && sym->sy_type->ty_class == T_CROSS
		    && sym->sy_type->ty_cross == (p_type)0 && sym->sy_type->ty_size == tp->ty_class
