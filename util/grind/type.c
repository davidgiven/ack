...
long float_size = sizeof(float), double_size = sizeof(double);
...
static struct bounds ibounds[2] = { { -128, 127 }, { -32768, 32767 } };

static struct bounds ubounds[2] = { { 0, 255 }, { 0, 65535 } };
...
p_type subrange_type(int A, int* base_index, long c1, long c2, int* result_index)
{
	...
	if (!A)
	{
		...
		if (result_index && result_index[0] == base_index[0] && result_index[1] == base_index[1])
		{
	...
	if (itself)
		...
		base_type = *(tp_lookup(base_index));
	...
	if (!A)
	{
		...
		*/
		if (itself || base_type == int_type)
		{
			...
			if (ip)
			{
				while (ip->maxval != 0 && ip->maxval != c2)
					ip++;
				if (ip->maxval)
					return ip->type;
			}
	...
	if (!A && p->ty_base == int_type)
	{
		...
		else if (ifit(p->ty_up, (int)short_size) && ifit(p->ty_low, (int)short_size))
		{
			p->ty_size = short_size;
		}
	}
...
static long nel(p_type tp)
{
	switch (tp->ty_class)
	{
		case T_SUBRANGE:
			if (tp->ty_A)
				return 0;
			if (tp->ty_low <= tp->ty_up)
				return tp->ty_up - tp->ty_low + 1;
...
void init_types(void)
{
	...
	i_types[0].maxval = max_int[(int)int_size];
	i_types[0].type = int_type;
	i_types[1].maxval = max_int[(int)short_size];
	i_types[1].type = short_type;
	i_types[2].maxval = max_int[(int)long_size];
	i_types[2].type = long_type;
	u_types[0].maxval = max_uns[(int)int_size];
	u_types[0].type = uint_type;
	u_types[1].maxval = max_uns[(int)short_size];
	u_types[1].type = ushort_type;
	u_types[2].maxval = max_uns[(int)long_size];
	u_types[2].type = ulong_type;
}
...
p_type* tp_lookup(int* type_index)
{
	...
	while (type_index[0] >= list_len)
	{
		if (list_len)
		{
			list_row = (struct tp_index*)realloc(
			    (char*)list_row, (list_len += NINCR) * sizeof(struct tp_index));
		}
		else
			list_row = (struct tp_index*)malloc((list_len = NINCR) * sizeof(struct tp_index));
		for (i = NINCR; i > 0; i--)
		{
	...
	while (type_index[1] >= p->len)
	{
		...
		if (p->len)
		{
			p->row = (p_type**)realloc((char*)p->row, (unsigned)(indx + 1) * sizeof(p_type*));
		}
		else
			p->row = (p_type**)malloc(sizeof(p_type*));
		p->len += NINCR;
		...
		while (--i >= 0)
		{
			...
			if (j)
			{
				while (--j > 0)
				{
					...
					{
						error(
						    "%s: incomplete type (%d,%d)", FileScope->sc_definedby->sy_idf->id_text,
						    i, j);
					}
				}
				j = list_row[i].len;
...
void end_literal(p_type tp, long maxval)
{
	...
	if (ufit(maxval, 1))
		tp->ty_size = 1;
	else if (ufit(maxval, (int)short_size))
		tp->ty_size = short_size;
	else
		tp->ty_size = int_size;
	if (!bool_type)
		bool_type = tp;
...
long param_size(int v, p_type t)
{
	if (v == 'i' || v == 'v')
	{
		/* addresss; only exception is a conformant array, which also
		   takes a descriptor.
		*/
		if (currlang == m2_dep && t->ty_class == T_ARRAY && t->ty_index->ty_class == T_SUBRANGE
		    && t->ty_index->ty_A)
		{
...
void add_param_type(int v, p_symbol s)
{
	...
	prc_type->ty_nparams++;
	prc_type->ty_params = (struct param*)realloc(
	    (char*)prc_type->ty_params, (unsigned)prc_type->ty_nparams * sizeof(struct param));
