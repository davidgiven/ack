...
#endif
#define N_STROFF(x)                                                                                \
	(N_TXTOFF(x) + (x).a_text + (x).a_data + (x).a_trsize + (x).a_drsize + (x).a_syms)
...
char* f;
{
	if ((inf = fopen(f, "rb")) == NULL)
		return 0;
	return 1;
}

rd_ohead(h) struct outhead* h;
{
	if (!readf(&bh, sizeof(struct exec), 1))
		rd_fatal();
	if (N_BADMAG(bh))
		rd_fatal();
...
/*ARGSUSED1*/
rd_name(names, count) struct outname* names;
unsigned int count; /* ignored; complete namelist is read */
{
	names->on_valu = 0;
	names->on_foff = seg_strings + OFF_CHAR(hh);
	names->on_desc = 0;
	names->on_type = S_MIN | S_SCT;
	names++;
	names->on_valu = 0;
	names->on_foff = seg_strings + OFF_CHAR(hh) + 6;
	names->on_desc = 0;
	names->on_type = (S_MIN + 2) | S_SCT;
	names++;
	names->on_valu = 0;
	names->on_foff = seg_strings + OFF_CHAR(hh) + 12;
	names->on_desc = 0;
	names->on_type = (S_MIN + 3) | S_SCT;
	names++;
	...
	while (count > 0)
	{
		...
		if (!readf(&n, sizeof(struct nlist), 1))
			rd_fatal();
		count--;
		names->on_desc = n.n_desc;
		if (n.n_un.n_strx - 4 < 0)
			names->on_foff = 0;
		else
			names->on_foff = OFF_CHAR(hh) - 4 + n.n_un.n_strx;
...
rd_string(strings, count) char* strings;
long count;
{
#if defined(sun)
	if (bh.a_magic == ZMAGIC)
	...
	if (!readf(strings, (int)count - 17, 1))
		rd_fatal();
	strings += count - 17;
	strcpy(strings, ".text");
	strings += 6;
...
#define getlong(val, p)                                                                            \
	(val = (*p++ & 0377), val |= (*p++ & 0377) << 8, val |= (*p++ & 0377L) << 16,                  \
	 val |= (*p++ & 0377L) << 24)
static get_names(h, sz) struct outhead* h;
long sz;
{
	char* xnms = malloc((unsigned)sz);
	char* p;
	struct outname* onm
	    = (struct outname*)malloc((((unsigned)sz + 8) / 9) * sizeof(struct outname));
	struct xnm
	{
...
rd_ohead(h) struct outhead* h;
{
	...
	if (!readf(&xhdr, sizeof(xhdr), 1))
		rd_fatal();
	if (xhdr.x_magic != XMAGIC)
		rd_fatal();
	h->oh_magic = O_CONVERTED;
	h->oh_stamp = 0;
	h->oh_nsect = 4;
	...
	fseek(inf, 0140L, 0);
	if (!readf(&xseg[0], sizeof(xseg[0]), sepid + 2))
		rd_fatal();
...
rd_name(nm, count) struct outname* nm;
unsigned int count;
{
	...
	while (dn < maxdn)
	{
		if (dn->n_type & N_STAB)
		{
			...
			if (dn->n_un.n_strx == 0)
				...
				n->on_foff = OFF_CHAR(hh) + dn->n_un.n_strx;
			n++;
		}
		dn++;
	}
