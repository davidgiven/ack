...
void lines(p_file file, int l1, int l2)
{
	...
	if (last_file != file)
	{
		if (last_f)
			fclose(last_f);
		last_f = 0;
		if (!(f = open_file(file->f_sym->sy_idf->id_text, "r", &file->f_fullname)))
		{
	...
	if (l1 > file->f_nlines)
		l1 = file->f_nlines;
	if (l1 + l2 - 1 > file->f_nlines)
		l2 = file->f_nlines - l1 + 1;
	...
	for (n = l1; n < l1 + l2; n++)
	{
		...
		if (interrupted)
			return;
		fprintf(
		    db_out, "%c%5d\t", currfile && file == currfile->sy_file && n == currline ? '>' : ' ',
		    n);
		do
		{
			c = getc(f);
...
static void line_positions(p_file file, FILE* f)
{
	...
	while ((c = getc(f)) != EOF)
	{
		...
		if (c == '\n')
		{
			...
			{
				n_alloc <<= 1;
				file->f_linepos = (long*)realloc((char*)(file->f_linepos), n_alloc * sizeof(long));
			}
			file->f_linepos[nl++] = cnt;
		}
	}
	if (cnt == file->f_linepos[nl - 1])
		nl--;
	file->f_linepos = (long*)realloc((char*)(file->f_linepos), (unsigned)nl * sizeof(long));
	file->f_nlines = nl;
	clearerr(f);
}
