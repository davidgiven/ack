	...
	*/
	...
	for (i = item_list.il_first; i != 0; i = i->i_next)
	{
		...
		if (!i->i_disabled && (p->t_address == a || p->t_address == NO_ADDR))
		{
			...
			{
				case OP_TRACE:
					if ((!stop_reason && mess_type != 0) || p->t_args[2] || !may_stop)
					{
						perform(p, a);
					}
					break;
...
void handle_displays(void)
{
	...
	while (i)
	{
		...
		if (!i->i_disabled && p->t_oper == OP_DISPLAY)
			do_print(p);
		i = i->i_next;
	}
}
...
void add_to_item_list(p_tree p)
{
	...
	i = new_item();
	i->i_node = p;
	if (p->t_address == NO_ADDR && (p->t_oper != OP_TRACE || !p->t_args[0]))
		db_ss++;
...
void remove_from_item_list(int n)
{
	...
	if (prev)
	{
		...
		item_list.il_first = i->i_next;
	if (i == item_list.il_last)
		item_list.il_last = prev;
	p = i->i_node;
	if (p->t_address == NO_ADDR && (p->t_oper != OP_TRACE || !p->t_args[0]))
		db_ss--;
	free_item(i);
...
void perform_items(void)
{
	...
	for (; i; i = i->i_next)
	{
		if (!i->i_disabled && i->i_node->t_oper != OP_DUMP)
			eval(i->i_node);
	}
}
