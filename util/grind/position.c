...
*/
p_position get_position_from_addr(t_addr t)
{
	...
	do
	{
		...
		while ((p->on_type >> 8) != N_SLINE)
			p++;
		if (p->on_valu > t)
		{
			p--;
			while (p > &f_start[i] && (p->on_type >> 8) != N_SLINE)
				p--;
			j = p - f_start;
		}
		else
	...
	p = &f_start[j - 1];
	while ((i = p->on_type >> 8) != N_SOL && i != N_SO)
		p--;
...
*/
t_addr get_addr_from_position(p)
p_position p;
{
...
*/
p_position print_position(a, print_function)
t_addr a;
int print_function;
{
	...
	if (pos)
		fprintf(db_out, "at \"%s\":%u", pos->filename, pos->lineno);
	return pos;
}
