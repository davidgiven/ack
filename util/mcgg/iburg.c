...
static void addonewayregalias(struct reg* r1, struct reg* r2)
{
...
{
	addonewayregalias(r1, r2);
	addonewayregalias(r2, r1);
}
...
static void emitregisterattrsqtable(void)
{
	...
	for (i = 0; i < registerattrs.count; i++)
	{
		...
		for (j = 0; j < registerattrs.count; j++)
		{
			...
			/* Given a pair of register classes, find out the maximum number of
			 * rc2 registers any given rc1 register can overlap. */

			for (k = 0; k < registers.count; k++)
			{
				int count = 0;

				struct reg* r1 = registers.item[k].right;
				if (r1->attrs & (1 << rc1->number))
				{
					for (l = 0; l < r1->aliases.count; l++)
					{
						struct reg* r2 = r1->aliases.item[l];
						if (r2->attrs & (1 << rc2->number))
							count++;
					}
				}
