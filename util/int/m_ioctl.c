...
int do_ioctl(int fd, int req, ptr addr)
{
	...
	switch (req)
	{
		...
		case TIOCGETP:
			...
			if ((e = ioctl(fd, req, (char*)&sg_buf)) == -1 || !sgttyb2mem(addr, &sg_buf))
			{
		...
		case TIOCSETN:
			...
			if (!mem2sgtty(addr, &sg_buf) || (e = ioctl(fd, req, (char*)&sg_buf)) == -1)
			{
				e = -1; /* errno already set */
			}
			break;
		...
		case TIOCGETC:
			...
			if ((e = ioctl(fd, req, (char*)&tc_buf)) == -1 || !tchars2mem(addr, &tc_buf))
			{
				e = -1; /* errno already set */
			}
			break;
		...
		case TIOCSETC:
			...
			if (!mem2tchars(addr, &tc_buf) || (e = ioctl(fd, req, (char*)&tc_buf)) == -1)
			{
				e = -1;
			}
			break;
		...
		case TIOCLGET: /* addr points to space, store lmw there */
			if (memfault(addr, wsize) || (e = ioctl(fd, req, (char*)&mask)) == -1)
			{
				e = -1;
			}
			else
		...
		case TIOCGLTC:
			...
			if ((e = ioctl(fd, req, (char*)&ltc_buf)) == -1 || !ltchars2mem(addr, &ltc_buf))
			{
				e = -1; /* errno already set */
			}
			break;
		...
		case TIOCSLTC:
			...
			if (!mem2ltchars(addr, &ltc_buf) || (e = ioctl(fd, req, (char*)&ltc_buf)) == -1)
			{
				e = -1;
			}
			break;
		...
		case TIOCGETD:
			...
			if (memfault(addr, wsize) || (e = ioctl(fd, req, (char*)&ldisc)) == -1)
			{
				e = -1;
			}
			else
		...
		case TIOCGPGRP:
			...
			if (memfault(addr, wsize) || (e = ioctl(fd, req, (char*)&pgrp)) == -1)
			{
				e = -1;
			}
			else
		...
		case FIONREAD: /* do the ioctl, addr is long-int ptr now */
			if (memfault(addr, wsize) || (e = ioctl(fd, req, (char*)&count)) == -1)
			{
				e = -1;
			}
			else
