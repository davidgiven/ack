...
{
	proctab = (struct proc*)Malloc(NProc * sizeof(struct proc), "proctable");
	pr_cnt = 0;
}
...
void add_proc(size nloc, ptr ep)
{
	...
	for (p = &proctab[0]; p < pr; p++)
	{
		if (/* the old one starts earlier */
		    p->pr_ep < pr->pr_ep && /* it seems to end later */
		    p->pr_ff > pr->pr_ep)
		{ /* update its limit */
			p->pr_ff = pr->pr_ep;
		}
		if (/* the old one starts later */
		    p->pr_ep > pr->pr_ep && /* our limit is beyond the old procedure entry point*/
		    ff > p->pr_ep)
		{ /* update our limit */
			ff = p->pr_ep;
		}
