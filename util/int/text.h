...
#define L_arg_2() (PC += 2, (btol(text_loc(PC - 1)) | (btos(text_loc(PC - 2)) << 8)))

#define P_arg_2()                                                                                  \
	(PC += 2, (btol(text_loc(PC - 1)) | (btos(text_loc(PC - 2)) << 8))) /* should test */

#define N_arg_2()                                                                                  \
	(PC += 2, (btol(text_loc(PC - 1)) | (btos(text_loc(PC - 2)) << 8))) /* should test */

#define U_arg() (PC += 2, (btol(text_loc(PC - 1)) | (btol(text_loc(PC - 2)) << 8)))
...
#define L_arg_4()                                                                                  \
	(PC += 4,                                                                                      \
	 (btol(text_loc(PC - 1)) | (btol(text_loc(PC - 2)) << 8) | (btol(text_loc(PC - 3)) << 16)      \
...
#define arg_c(n) ((n < i_minsw || n > i_maxsw) ? (wtrap(WARGC, EILLINS), 0) : n)

#define arg_d(n) ((wsize > 2) ? (wtrap(WARGD, EILLINS), 0) : n)

#define arg_l(n) ((n < min_off || n > max_off) ? (wtrap(WARGL, EILLINS), 0) : n)

#define arg_g(p) ((p >= HB) ? (wtrap(WARGG, EILLINS), i2p(0)) : p)

#define arg_f(n) ((n < min_off || n > max_off) ? (wtrap(WARGF, EILLINS), 0) : n)

#define arg_n(u) ((u > i_maxuw) ? (wtrap(WARGL, EILLINS), 0) : u)

#define arg_s(s) ((s <= 0 || s > max_off || (s & wsizem1)) ? (trap(EODDZ), s) : s)

#define arg_z(s) ((s < 0 || s > max_off || (s & wsizem1)) ? (trap(EODDZ), s) : s)

#define arg_o(s) ((s < 0 || s > max_off || ((s & wsizem1) && wsize % s)) ? (trap(EODDZ), s) : s)

#define arg_w(s) ((s <= 0 || s > max_off || (s & wsizem1)) ? (trap(EODDZ), s) : s)
...
/* tests on widths */
#define arg_wn(s) ((s != 1 && s != 2 && s != 4) ? (trap(EODDZ), s) : s)

#define arg_wf(s) ((s != 4 && s != 8) ? (trap(EODDZ), s) : s)

#define arg_wi(s) (((s != 2 && s != 4) || (s & wsizem1)) ? (trap(EODDZ), s) : s)
