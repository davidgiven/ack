...
static void check_requirements(char* name)
{
	...
	{
		fprintf(
		    stderr, "%s compiled with compiler that does not support signed right shifts. Aborted.",
		    name);
		exit(1);
	}
}
...
int main(int argc, char* argv[])
{
	...
	if (must_disassemble)
	{
		message("text segment disassembly produced; program was not run");
		disassemble();
		close_down(0);
	}
	...
	while (running)
	{
		...
		LOG(
		    ("@x9 PC = %lu OPCODE = %lu", PC,
		     btol(text_loc(PC)) < SECONDARY ? btol(text_loc(PC))
