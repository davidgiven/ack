...
double str2double(char* str)
{
	...
	while ((b = *str++) != 'e' && b != 'E' && b != '\0')
	{
		...
		else if ('0' <= b && b <= '9')
		{
			...
			if (frac)
			{
				mantissa = adf(mantissa, ttttp(bval, -frac));
				frac++;
			}
			else
			{
				mantissa = adf(mlf(mantissa, 10.0), bval);
			}
		}
		else
		{
		...
		LOG((" q9 str2double : (inside while) mantissa = %20.20g", mantissa));
	}
