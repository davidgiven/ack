...
#ifdef SYS_V
struct timeb
{ /* non-existing; we use an ad-hoc definition */
	long time;
	...
	switch (pop_int())
	{
...
#ifdef LOGGING
			ES_def = ((st_sh(SP) == UNDEFINED) || (st_sh(SP + wsize - 1) == UNDEFINED)) ? UNDEFINED
			                                                                            : DEFINED;
#else
			ES_def = DEFINED;
#endif /* LOGGING */
		...
		case 2: /* Fork */
			...
			else
			{
				/* fork call failed */
				push_err();
				LOG(("@m4 Fork: failed, pid = %d, errno = %d", pid, errno));
			}
			break;
		...
		case 3: /* Read */
			...
			for (nr = n, cp = buf[0], addr = dsp1; nr; nr--, addr++, cp++)
			{
				if (in_stack(addr))
				{
					ch_st_prot(addr);
		...
		case 4: /* Write */
			...
			check_buf(0, (size)nbytes);
			for (nr = nbytes, addr = dsp1, cp = buf[0]; nr; nr--, addr++, cp++)
			{
				*cp = mem_loc(addr);
			}
		...
		write_error:
			push_err();
			LOG(("@m4 Write: failed, n = %d, nbytes = %d, errno = %d", n, nbytes, errno));
			break;
		...
		case 5: /* Open */
			...
			if (!savestr(0, dsp1) || (fd = open(buf[0], flag)) == -1)
			{
				push_err();
				LOG(
				    ("@m4 Open: failed, file = %lu, flag = %d, fd = %d, errno = %d", dsp1, flag, fd,
				     errno));
			}
			else
			{
				push_int(fd);
				push_int(0);
				LOG(("@m9 Open: succeeded, file = %lu, flag = %d, fd = %d", dsp1, flag, fd));
			}
			break;
		...
		case 6: /* Close */
			...
			{
				push_err();
				LOG(("@m4 Close: failed, fd = %d, errno = %d", fd, errno));
			}
			else
			{
				push_int(0);
		...
		case 7: /* Wait */
			...
			{
				push_err();
				LOG(("@m4 Wait: failed, status = %d, errno = %d", status, errno));
			}
			else
			{
				push_i2(pid);
				push_i2(status);
				push_int(0);
				LOG(("@m9 Wait: succeeded, status = %d, pid = %d", status, pid));
			}
			break;
		...
		case 8: /* Creat */
			...
			{
				push_err();
				LOG(("@m4 Creat: failed, dsp1 = %lu, flag = %d, errno = %d", dsp1, flag, errno));
			}
			else
			{
				push_int(fd);
		...
		case 9: /* Link */
			...
			dsp1 = pop_ptr();
			dsp2 = pop_ptr();
			if (!savestr(0, dsp1) || !savestr(1, dsp2) || link(buf[0], buf[1]) == -1)
			{
				push_err();
			...
			{
				push_int(0);
				LOG(("@m9 Link: succeeded, dsp1 = %lu, dsp2 = %lu", dsp1, dsp2));
			}
			break;
		...
		case 10: /* Unlink */
			...
			{
				push_err();
				LOG(("@m4 Unlink: failed, dsp1 = %lu, errno = %d", dsp1, errno));
			}
			else
			{
				push_int(0);
		...
		case 12: /* Chdir */
			...
			{
				push_err();
				LOG(("@m4 Chdir: failed, dsp1 = %lu, errno = %d", dsp1, errno));
			}
			else
			{
				push_int(0);
		...
		case 14: /* Mknod */
			...
			if (!savestr(0, dsp1) || mknod(buf[0], mode, address) == -1)
			{
				push_err();
				LOG(
				    ("@m4 Mknod: failed, dsp1 = %lu, mode = %d, address = %d, errno = %d", dsp1,
				     mode, address, errno));
			}
			else
			{
		...
		case 15: /* Chmod */
			...
			{
				push_err();
				LOG(("@m4 Chmod: failed, dsp1 = %lu, mode = %d, errno = %d", dsp1, mode, errno));
			}
			else
			{
				push_int(0);
		...
		case 16: /* Chown */
			...
			if (!savestr(0, dsp1) || chown(buf[0], owner, group) == -1)
			{
				push_err();
				LOG(
				    ("@m4 Chown: failed, dsp1 = %lu, owner = %d, group = %d, errno = %d", dsp1,
				     owner, group, errno));
			}
			else
			{
		...
		case 18: /* Stat */
			...
			if (!savestr(0, dsp1) || stat(buf[0], &st_buf) == -1 || !stat2mem(dsp2, &st_buf))
			{
				push_err();
				LOG(("@m4 Stat: failed, dsp1 = %lu, dsp2 = %lu, errno = %d", dsp1, dsp2, errno));
			}
			else
			{
				push_int(0);
				LOG(("@m9 Stat: succeeded, dsp1 = %lu, dsp2 = %lu", dsp1, dsp2));
			}
			break;
		...
		case 19: /* Lseek */
			...
			off = pop_int4();
			whence = pop_int();
			LOG(("@m4 Lseek: fd = %d, off = %ld, whence = %d", fd, off, whence));
		...
		case 21: /* Mount */
			...
			dsp2 = pop_ptr();
			flag = pop_int();
			if (!savestr(0, dsp1) || !savestr(1, dsp2) || mount(buf[0], buf[1], flag) == -1)
			{
				push_err();
				LOG(
				    ("@m4 Mount: failed, dsp1 = %lu, dsp2 = %lu, flag = %d, errno = %d", dsp1, dsp2,
				     flag, errno));
			}
			else
			{
				push_int(0);
				LOG(("@m9 Mount: succeeded, dsp1 = %lu, dsp2 = %lu, flag = %d", dsp1, dsp2, flag));
			}
			break;
		...
		case 22: /* Umount */
			...
			{
				push_err();
				LOG(("@m4 Umount: failed, dsp1 = %lu, errno = %d", dsp1, errno));
			}
			else
			{
				push_int(0);
		...
		case 23: /* Setuid */
			...
			{
				push_err();
				LOG(("@m4 Setuid: failed, userid = %d, errno = %d", userid, errno));
			}
			else
			{
				push_int(0);
		...
		case 25: /* Stime */
			...
			{
				push_err();
				LOG(("@m4 Stime: failed, tm = %ld, errno = %d", tm, errno));
			}
			else
			{
				push_int(0);
		...
		case 26: /* Ptrace */
			...
			push_err();
			LOG(
			    ("@m4 Ptrace: failed, request = %d, pid = %d, addr = %lu, data = %d, errno = %d",
		...
		case 28: /* Fstat */
			...
			fd = pop_int();
			dsp2 = pop_ptr();
			if (!good_fd(fd) || fstat(fd, &st_buf) == -1 || !stat2mem(dsp2, &st_buf))
			{
				push_err();
				LOG(("@m4 Fstat: failed, fd = %d, dsp2 = %lu, errno = %d", fd, dsp2, errno));
			}
			else
			{
				push_int(0);
				LOG(("@m9 Fstat: succeeded, fd = %d, dsp2 = %lu", fd, dsp2));
			}
			break;
		...
		case 30: /* Utime */
			...
			{
				push_err();
				LOG(("@m4 Utime: failed, dsp1 = %lu, dsp2 = %lu, errno = %d", dsp1, dsp2, errno));
				break;
			}
			...
			{
				push_int(0);
				LOG(("@m9 Utime: succeeded, dsp1 = %lu, dsp2 = %lu", dsp1, dsp2));
			}
			break;
		...
		case 33: /* Access */
			...
			{
				push_err();
				LOG(("@m4 Access: failed, dsp1 = %lu, mode = %d, errno = %d", dsp1, mode, errno));
			}
			else
			{
				push_int(0);
				LOG(("@m9 Access: succeeded, dsp1 = %lu, mode = %d", dsp1, mode));
			}
			break;
		...
		case 35: /* Ftime */
			...
			{
				push_err();
				LOG(("@m4 Ftime: failed, dsp2 = %lu, errno = %d", dsp2, errno));
			}
			else
			{
				push_int(0);
		...
		case 37: /* Kill */
			...
			{
				push_err();
				LOG(("@m4 Kill: failed, pid = %d, sig = %d, errno = %d", pid, sig, errno));
			}
			else
			{
				push_int(0);
				LOG(("@m9 Kill: succeeded, pid = %d, sig = %d", pid, sig));
			}
			break;
		...
		case 41: /* Dup & Dup2 */
			...
			{
				push_err();
				LOG(("@m4 Dup/Dup2: failed, fdnew = %d, errno = %d", fdnew, errno));
			}
			else
			{
				push_int(fdnew);
		...
		case 42: /* Pipe */
			...
			else
			{
				...
				push_int(pfds[1]);
				push_int(0);
				LOG(("@m9 Pipe: succeeded, pfds[0] = %d, pfds[1] = %d", pfds[0], pfds[1]));
			}
			break;
		...
		case 43: /* Times */
			...
			{
				push_err();
				LOG(("@m4 Times: failed, dsp2 = %lu, errno = %d", dsp2, errno));
			}
			else
			{
		...
		case 44: /* Profil */
			...
			push_err();
			LOG(
			    ("@m4 Profil: failed, dsp1 = %lu, nbytes = %d, offset = %d, scale = %d, errno = %d",
		...
		case 47: /* Getgid */
			...
			groupid = getgid();
			push_i2(groupid);
			LOG(("@m9 Getgid(part 1): succeeded, real groupid = %d", groupid));
			groupid = getegid();
			push_i2(groupid);
			LOG(("@m9 Getgid(part 2): succeeded, eff groupid = %d", groupid));
			break;
		...
		case 48: /* Sigtrp */
			...
			if ((old_trap_no = do_sigtrp(trap_no, sig_no)) == -1)
			{
				push_err();
				LOG(
				    ("@m4 Sigtrp: failed, trap_no = %d, sig_no = %d, errno = %d", trap_no, sig_no,
				     errno));
			}
			else
			{
				...
				push_int(0);
				LOG(
				    ("@m9 Sigtrp: succeeded, trap_no = %d, sig_no = %d, old_trap_no = %d", trap_no,
				     sig_no, old_trap_no));
			}
			break;
		...
		case 51: /* Acct */
			...
			einval(WMPXIMP);
			push_err();
			LOG(("@m4 Acct: failed, request = %d, dsp1 = %lu, errno = %d", request, dsp1, errno));
#endif
			break;
		...
		case 54: /* Ioctl */
			...
			if (!good_fd(fd) || do_ioctl(fd, request, dsp2) != 0)
			{
				push_err();
				LOG(
				    ("@m4 Ioctl: failed, fd = %d, request = %d, dsp2 = %lu, errno = %d", fd,
				     request, dsp2, errno));
			}
			else
			{
				push_int(0);
				LOG(("@m9 Ioctl: succeeded, fd = %d, request = %d, dsp2 = %lu", fd, request, dsp2));
			}
			break;
		...
		case 56: /* Mpxcall */
			...
			push_err();
			LOG((
			    "@m4 Mpxcall: failed, request = %d, dsp1 = %lu, errno = %d", request, dsp1, errno));
			break;
		...
		case 59: /* Exec */
			...
			dsp2 = pop_ptr();
			dsp3 = pop_ptr();
			if (!savestr(0, dsp1) || !vec(1, 2, dsp2, &argvec) || !vec(3, 4, dsp3, &envvec)
			    || /* execute results, ignore return code */
			    (execve(buf[0], argvec, envvec), 1))
			{
				push_err();
				LOG(
				    ("@m4 Exece: failed, dsp1 = %lu, dsp2 = %lu, dsp2 = %lu, errno = %d", dsp1,
				     dsp2, dsp3, errno));
			}
			break;
		...
		case 60: /* Umask */
			...
			push_int(oldmask);
			LOG(("@m9 Umask: succeeded, mode = %d, oldmask = %d", mode, oldmask));
			break;
...
static int vec(int n1, int n2, ptr addr, char*** vecvec)
{
	...
	*vecvec = (char**)Malloc((size)(n_ent * sizeof(char*)), "argvec or envvec in exec()");
	check_buf(n1, str);

	/* copy the elements */
	for (cp1 = buf[n1], n_ent = 0, p = addr; (ldp = mem_lddp(p)) != 0; p += psize, n_ent++)
	{
		if (!savestr(n2, ldp))
		{
			return 0;
