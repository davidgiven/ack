...
void do_crs(struct outname* base, unsigned int count)
{
	...
	while (count--)
	{
		if ((name->on_type & S_TYP) == S_CRS)
		{
			...
			if (flagword & RFLAG)
			{
				name->on_valu = NLocals + (p - (struct outname*)address(ALLOGLOB, (ind_t)0));
			}
			else
			{
...
static void handle_relos(struct outhead* head, struct outsect* sects, struct outname* names)
{
	...
	else
	{
		for (sectindex = 0; sectindex < head->oh_nsect; sectindex++)
		{
			if (sects[sectindex].os_flen)
			{
				...
				else
				{
					...
					while (sz)
					{
						...
						{
							relo = nextrelo();
							if (relo->or_sect - S_MIN == sectindex && relo->or_addr >= sf
							    && relo->or_addr < sf + sz2)
							{
								relocate(head, emit, names, relo, sf);
								/*
								 * Write out the (probably changed)
								 * relocation information.
								 */
			...
			zeros[sectindex] += sects[sectindex].os_size - sects[sectindex].os_flen;
		}
	}
}
