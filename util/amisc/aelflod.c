...
const char elf_le_ident_string[] = { 0x7F, 'E', 'L', 'F' };
...
const char shstrtab[] = "\0.text\0.rodata\0.data\0.bss\0.stab\0.stabstr\0"
                        ".symtab\0.strtab\0.shstrtab";
/* Compiler appends one more "\0". */
const int sh_name[] = {
	/* Index of each name in shstrtab: */
	0, 1, 7, 15, 21, 26, 32, 41, 49, 57,
};
...
void emit_symtab(void)
{
	...
	for (pass = 0; pass < 2; pass++)
	{
		for (i = 0; i < outhead.oh_nname; i++)
		{
			...
			if ((pass == 0 && !global) || (pass == 1 && global))
			{
				emit32(cvname(n)); /* name index */
				emit32(n->on_valu); /* value */
...
void emit_sh(int i)
{
	...
	switch (i)
	{
		...
		case N_DATA:
			...
			offset = code_offset + outsect[TEXT].os_size + outsect[ROM].os_size;
...
int rnames(FILE* f)
{
	...
	for (i = 0; i < outhead.oh_nname; i++)
	{
		...
		if (fread(buf, SZ_NAME, 1, f) != 1)
			return 0;
		c = buf;
		outname[i].on_foff = get4(c);
		c += 4;
		outname[i].on_type = uget2(c);
		c += 2;
		outname[i].on_desc = uget2(c);
		c += 2;
...
int main(int argc, char* argv[])
{
	...
	while ((argc > 1) && (argv[1][0] == '-'))
	{
		switch (argv[1][1])
		{
			...
			case 'h':
				fprintf(
				    stderr,
				    "%s: Syntax: aelflod [-a<number>] [-b] [-h] [-l]\n\t[-m<number>] [-v] "
	...
	stab_offset
	    = code_offset + outsect[TEXT].os_size + outsect[ROM].os_size + outsect[DATA].os_size;
	...
	if (outhead.oh_nname)
	{
		...
		if (nstab == 0)
		{
			...
			sh_count -= 2;
			shstrtab_nr -= 2;
			shstrtab_size -= (sh_name[N_SYMTAB] - sh_name[N_STAB]);
		}
...

	{
		...
		emitphdr(outsect[TEXT].os_base & ~0x1FFF, filelength, memlength, 0, 4 | 2 | 1);
	}
