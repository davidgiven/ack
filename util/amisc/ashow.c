...
{
	if (flagword & HF_LINK)
		printf("unresolved references left\n");
}
...
static void showrelo(void)
{
	...
	if (relrec.or_type & RELPC)
		printf("\tpc relative\n");
	if (relrec.or_type & RELBR)
		printf("\tbytes reversed\n");
	if (relrec.or_type & RELWR)
