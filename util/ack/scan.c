	...
	scanlist(f_scan, scan)
	{
		...
		if (satisfy(trafo, suffix))
		{
			...
			if (trafo->t_prep)
			{
				...
				if (stopsuffix && strcmp(stopsuffix, cpp_trafo->t_out) == 0)
				{
					scan_found();
					return;
				}
...
#ifdef DEBUG
			if (debug >= 4)
				vprint("%s-", t_cont(*scan)->t_name);
#endif
			if (t_cont(*scan)->t_optim)
				ocount++;
			else
				ncount++;
			if (t_cont(*scan)->t_optim > Optlevel)
				pcount--;
...
static int satisfy(trf* trafo, const char* suffix)
{
	...
	*/
	...
	while (l_char)
	{
		...
		{
			fuerror("Illegal input suffix entry for %s", trafo->t_name);
		}
...
static enum f_path scan_end(trf** first)
{ /* Finalization */
...
#endif
	if (last_ncount == -1)
		return suf_found ? F_NOPATH : F_NOMATCH;
#ifdef DEBUG
	if (debug >= 2)
		vprint("Transformation found\n");
#endif
	prev = (trf*)0;
	*first = prev;
	scanlist(l_first(tr_list), scan)
	...
	if (cpp_trafo && stopsuffix && strcmp(stopsuffix, cpp_trafo->t_out) == 0)
	{
...
static void find_cpp(void)
{
	...
	scanlist(l_first(tr_list), elem)
	{
		...
		{
			if (cpp_trafo)
				fuerror("Multiple cpp's present");
			cpp_trafo = t_cont(*elem);
		}
	}
	if (!cpp_trafo)
		fuerror("No cpp present");
}
