...
static void intrf(void)
{
	...
	for (;;)
	{
		...
		else if (strcmp(ty_name, OUT) == 0)
		{
			if (new->t_out)
				twice = YES;
			new->t_out = keeps(bol);
		}
		else if (strcmp(ty_name, PROG) == 0)
		{
			if (new->t_prog)
				twice = YES;
			new->t_prog = keeps(bol);
		}
		else if (strcmp(ty_name, MAPF) == 0)
		{
			l_add(&new->t_mapf, keeps(bol));
		}
		else if (strcmp(ty_name, ARGS) == 0)
		{
			if (new->t_argd)
				twice = YES;
			new->t_argd = keeps(bol);
		}
		else if (strcmp(ty_name, STD_IN) == 0)
		{
			if (new->t_stdin)
				twice = YES;
			new->t_stdin = YES;
		}
		else if (strcmp(ty_name, STD_OUT) == 0)
		{
			if (new->t_stdout)
				twice = YES;
			new->t_stdout = YES;
		}
		else if (strcmp(ty_name, PREP) == 0)
		{
			...
			else if (strcmp(bol, "cond") == 0)
			{
				if (new->t_prep)
					twice = YES;
				new->t_prep = MAYBE;
			}
			else if (strcmp(bol, "is") == 0)
			{
				if (new->t_isprep)
					twice = YES;
				new->t_isprep = YES;
			...
			{
				fuerror("illegal preprocessor spec in %s: %s", inname, bol);
			}
		}
		else if (strcmp(ty_name, OPT) == 0)
		{
			if (new->t_optim)
				twice = YES;
		...
		else if (strcmp(ty_name, LINKER) == 0)
		{
			if (new->t_linker)
				twice = YES;
			new->t_linker = YES;
		...
		else if (strcmp(ty_name, COMBINER) == 0)
		{
			if (new->t_combine)
				twice = YES;
			new->t_combine = YES;
		}
		else if (strcmp(ty_name, PRIO) == 0)
		{
			new->t_priority = atoi(bol);
		}
		else if (strcmp(ty_name, RUNT) == 0)
		{
			if (new->t_rts)
				twice = YES;
			new->t_rts = keeps(bol);
		}
		else if (strcmp(ty_name, NEEDT) == 0)
		{
			if (new->t_needed)
				twice = YES;
			new->t_needed = keeps(bol);
		}
		else if (strcmp(ty_name, RES) == 0)
		{
			if (new->t_outfile)
				twice = YES;
			new->t_outfile = keeps(bol);
		}
		else if (strcmp(ty_name, CALL) == 0)
		{
			if (callname && strcmp(bol, callname) == 0)
			{
				name_seen = 1;
		...
		if (twice)
		{
			werror("%s: specified twice for %s", ty_name, new->t_name);
		}
	}
	if (!(new->t_name&& new->t_out&& new->t_prog))
	{
		fuerror("insufficient specification for %s in %s", new->t_name, inname);
	}
	if (!new->t_argd)
		new->t_argd = "";
	/* Warning, side effect */
	if (name_seen && new->t_rts)
	{
		...
		{
			error("Attempt to use two run-time systems, %s and %s", rts, new->t_rts);
		}
		rts = new->t_rts;
		l_add(&head_list, rts);
		l_add(&tail_list, rts);
	...
	if (debug >= 3)
	{
		list_elem* elem;
		vprint(
		    "%s: from %s to %s '%s'\n", new->t_name, new->t_in ? new->t_in : "(null)", new->t_out,
		    new->t_prog);
...
#endif
	if (new->t_optim && (new->t_optim <= Optlevel || inoptlist(new->t_name)))
	{
