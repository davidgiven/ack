...
static struct freeblock freexx[2] = { { freexx, 0 }, { freexx + 1, 0 } };
...
area_t getarea(unsigned int size)
{
	...
	for (;;)
	{
		for (l_ptr = &freehead, c_ptr = freehead.f_next; c_ptr != &freehead; c_ptr = c_ptr->f_next)
		{
