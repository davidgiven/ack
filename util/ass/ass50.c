...
void pass_5(void)
{
	...
	for (lnp = pstate.s_fline; lnp; lnp = lnp->l_next, line_num++)
	{
		...
		if (!defined && partype == PAR_G)
		{ /* must be external */
			text_reloc(
			    (lnp->type1 == GLOSYM ? lnp->ad.ad_gp : lnp->ad.ad_df.df_gp),
			    (FOFFSET)(textbytes + afterlength - inslength), op_curr - opchoice);
	...
	textbytes += prog_size;
	if (textbytes > maxadr)
		fatal("Maximum code area size exceeded");
	...
	for (; r; r = r->r_next)
	{
		if (r->r_typ == RELLOC)
		{
			...
			if (k->l_defined == YES)
				...
				error("case label at line %d undefined", k->l_min);
		}
	}
}
