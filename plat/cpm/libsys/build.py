...
generated = (
    [
        simplerule(
            ...
            outs=[f"={b}.s"],
            commands=[f"$[ins[0]] {n} {b} > $[outs]"],
        )
        for n, b in bdos_calls.items()
    ]
    + [
        simplerule(
            name=f"{b}",
    ...
    + [
        simplerule(
            ...
            ins=["./make_trap.sh"],
            outs=[f"={t}.s"],
            commands=[f"$[ins[0]] {t.lower()} {t} > $[outs]"],
        )
        for t in trap_calls
    ]
)
...
ackclibrary(
    ...
    deps=["lang/cem/libcc.ansi/headers", "plat/cpm/include"],
)
