...
PUBLIC void sys_getsp(proc, newsp) int proc; /* which proc has enabled signals */
...
PUBLIC void sys_exec(proc, ptr, traced) int proc; /* proc that did exec */
...
PUBLIC void sys_newmap(proc, ptr) int proc; /* proc whose map is to be changed */
...
PUBLIC void sys_copy(mptr) message* mptr; /* pointer to message */
{
	...
	if (_sendrec(SYSTASK, mptr) != 0)
		panic("sys_copy can't send", NO_NUM);
}

PUBLIC void sys_times(proc, ptr) int proc; /* proc whose times are needed */
...
#if (CHIP == M68000)
PUBLIC void sys_fresh(proc, ptr, dc, basep, sizep) int proc; /* proc whose map is to be changed */
char* ptr; /* pointer to new map */
...
PUBLIC void sys_kill(proc, sig) int proc; /* which proc has exited */
int sig; /* signal number: 1 - 16 */
{
...
PUBLIC void tell_fs(what, p1, p2, p3) int what, p1, p2, p3;
{
