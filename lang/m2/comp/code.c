			...
			switch (nd->nd_symb)
			{
				case REAL:
					...
					if (!nd->nd_RSTR)
					{
						...
						C_rom_fcon(nd->nd_RSTR, tp->tp_size);
					c_lae_dlb(data_label);
					C_loi(tp->tp_size);
					break;
				case STRING:
...
static void CodeStd(struct node* nd)
{
	...
	switch (std)
	{
		...
		case S_INC:
		{
			...
			if ((int)size == (int)word_size)
			{
				RangeCheck(left->nd_type, tp->tp_fund == T_INTEGER ? int_type : card_type);
			}
			if (compl)
			{
				LOL(tmp, pointer_size);
...
)
{
	...
	switch (expr->nd_symb)
	{
		...
		case '*':
			...
			switch (fund)
			{
				...
				case T_INTORCARD:
					if (!options['R'])
					{
						C_cal((int)(size) <= (int)word_size ? "muluchk" : "mululchk");
					}
					C_mlu(size);
					break;
				case T_REAL:
		...
		case DIV:
			...
			{
				case T_INTEGER:
					C_cal((int)(size) == (int)word_size ? "dvi" : "dvil");
					C_asp(2 * size);
					C_lfr(size);
					break;
				case T_POINTER:
		...
		case MOD:
			...
			{
				case T_INTEGER:
					C_cal((int)(size) == (int)word_size ? "rmi" : "rmil");
					C_asp(2 * size);
					C_lfr(size);
					break;
				case T_POINTER:
...
static void CodeDAddress(struct node* nd, int chk_controlvar)
{
	...
	*/
	...
	chkptr = designator.dsg_kind == DSG_PLOADED || designator.dsg_kind == DSG_PFIXED;
