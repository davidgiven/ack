		...
		case T_PROCEDURE:
			...
			stb_type(tp->tp_next ? tp->tp_next : void_type, 0);
			{
				...
				while (p)
				{
					addc_db_str(
					    IsVarParam(p)                           ? 'v'
					        : IsConformantArray(TypeOfParam(p)) ? 'i'
					                                            : 'p');
...
void stb_addtp(char* s, struct type* tp)
{
	...
	addc_db_str(';');
	C_ms_stb_cst(
	    db_str.base, N_LSYM, tp == void_type || tp->tp_size >= max_int[2] ? 0 : (int)tp->tp_size,
	    (arith)0);
}
	...
	switch (kind)
	{
		...
		case D_PROCEDURE:
			...
			if (gdb_flag)
			{
				...
				addc_db_str(';');
			C_ms_stb_pnam(
			    db_str.base, N_FUN, gdb_flag ? 0 : proclevel, df->prc_vis->sc_scope->sc_name);
			break;
		case D_END:
			if (gdb_flag)
				break;
		...
		case D_VARIABLE:
			...
			else if (df->df_flags & D_VALPAR)
			{ /* value parameter */
				addc_db_str(IsConformantArray(tp) ? 'i' : 'p');
				stb_type(tp, 0);
				addc_db_str(';');
				C_ms_stb_cst(db_str.base, N_PSYM, 0, df->var_off);
			}
			else if (!proclevel || (df->df_flags & D_ADDRGIVEN))
			{ /* global */
				int knd = N_LCSYM;
		...
		case D_TYPE:
			...
			C_ms_stb_cst(
			    db_str.base, N_LSYM,
			    tp == void_type || tp->tp_size >= max_int[2] ? 0 : (int)tp->tp_size, (arith)0);
			break;
		case D_CONST:
			if (DefinitionModule && CurrVis != Defined->mod_vis)
				break;
			...
			switch (tp->tp_fund)
			{
				...
				case T_REAL:
					...
					if (!df->con_const.TOK_RSTR)
					{
						...
						adds_db_str(df->con_const.TOK_RSTR);
					addc_db_str(';');
					break;
				case T_STRING:
				{
					char* p = df->con_const.TOK_STR;
			...
			C_ms_stb_cst(
			    db_str.base, N_LSYM, tp->tp_size < max_int[2] ? (int)tp->tp_size : 0, (arith)0);
			break;
	}
}
