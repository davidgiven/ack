...
static struct string* GetString(int upto)
{
	 ...
	 */
	int ch;
	struct string* str = (struct string*)malloc((unsigned)sizeof(struct string));
	char* p;
	int len;
	...
	while (LoadChar(ch), ch != upto)
	{
		...
		*p++ = ch;
		if (p - str->s_str == len)
		{
			str->s_str = realloc(str->s_str, (unsigned int)len + RSTRSIZE);
			p = str->s_str + len;
			len += RSTRSIZE;
		}
	}
...
again:
	...
	switch (class(ch))
	{
		...
		case STSIMP:
			...
			if (ch == '&')
				return tk->tk_symb = AND;
			if (ch == '~')
				return tk->tk_symb = NOT;
		...
		case STNUM:
		{
					...
					case End:
					{
						...
						*np = '\0';
						...
						else
						{
							...
							*/
							arith ubound = max_int[sizeof(arith)] / (base >> 1);
							np = &buf[0];
							while (*np == '0')
								np++;
							tk->TOK_INT = 0;
							while (*np)
							{
								...
								if (tk->TOK_INT < 0 || tk->TOK_INT > ubound)
								{
									ovfl++;
								}
								tk->TOK_INT = tk->TOK_INT * base;
								if (tk->TOK_INT < 0 && tk->TOK_INT + c >= 0)
								{
									ovfl++;
								}
								tk->TOK_INT += c;
						...
						{
							toktype = char_type;
							if (ovfl != 0 || tk->TOK_INT > 255 || tk->TOK_INT < 0)
							{
						...
						if (ch == 'D' && (options['l'] || base == 10))
						{
								...
								*/
								    ...
								    && tk->TOK_INT <= max_int[(int)long_size])
								{
									toktype = longintorcard_type;
								             ...
								             tk->TOK_INT, full_mask[(int)long_size], T_CARDINAL))
								{
									ovfl = 1;
								}
							}
							...
							{
								if (ovfl != 0 || tk->TOK_INT > max_int[(int)long_size]
								    || tk->TOK_INT < 0)
								{
									ovfl = 1;
								}
						    ...
						    ovfl == 0 && tk->TOK_INT >= 0 && tk->TOK_INT <= max_int[(int)int_size])
						{
							toktype = intorcard_type;
						}
						else if (!chk_bounds(tk->TOK_INT, full_mask[(int)int_size], T_CARDINAL))
						{
							ovfl = 1;
						}
						if (ovfl)
			...
			*np++ = '\0';
			...
			if (np >= &buf[NUMSIZE])
			{
				tk->TOK_RSTR = strdup("0.0");
				lexerror("real constant too long");
			}
			else
				tk->TOK_RSTR = strdup(buf);
			CheckForLet();
