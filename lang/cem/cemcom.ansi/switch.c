...
void code_endswitch(void)
{
	...
	else
	{
		...
		if (compact(sh->sh_nrofentries, sh->sh_lowerbd, sh->sh_upperbd))
		{
			...
			C_rom_icon(writh2str(sh->sh_upperbd - sh->sh_lowerbd, 0), size);
			ce = sh->sh_entries;
	...
	for (ce = sh->sh_entries; ce;)
	{ /* free allocated switch structure */
...
void code_case(struct expr* expr)
{
	...
	{ /* second etc. case entry; put ce into proper place */
		struct case_entry *c1 = sh->sh_entries, *c2 = 0;

		if (val < sh->sh_lowerbd)
			sh->sh_lowerbd = val;
		else if (val > sh->sh_upperbd)
			sh->sh_upperbd = val;
		...
		*/
		if (c1)
		{
			...
			{
				error("multiple case entry for value %ld", ce->ce_value);
				free_case_entry(ce);
				return;
			}
			if (c2)
