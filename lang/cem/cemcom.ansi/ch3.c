	...
	*/
	...
	if (oper == ARROW)
	{
		if (tp->tp_fund == POINTER
		    && (tp->tp_up->tp_fund == STRUCT || tp->tp_up->tp_fund == UNION)) /* normal case */
			tp = tp->tp_up;
		else
		{ /* constructions like "12->selector" and
		  "char c; c->selector"
	  */
			switch (tp->tp_fund)
			{
				...
				case LNGLNG:
					...
					break;
				default:
					expr_error(exp, "-> applied to %s", symbol2str(tp->tp_fund));
				case ERRONEOUS:
	...
	switch (tp->tp_fund)
	{
		...
		default:
			if (!is_anon_idf(idf))
				expr_error(exp, "selector %s applied to %s", idf->id_text, symbol2str(tp->tp_fund));
		case ERRONEOUS:
			exp->ex_type = error_type;
			return;
	}
	...
	if (oper == '.')
	{
		...
		*/
		...
		else if (exp->ex_class == Oper)
		{
			...
			{
				exp = new_oper(sd->sd_type, exp, '.', intexpr(sd->sd_offset, INT));
	...
	{ /* oper == ARROW */
		...
		{
			exp = new_oper(sd->sd_type, exp, oper, intexpr(sd->sd_offset, INT));
		}
		exp->ex_lvalue = (sd->sd_type->tp_fund != ARRAY);
		exp->ex_flags &= ~EX_ILVALUE;
	}
	if ((sd->sd_type->tp_typequal & TQ_CONST) || (tp->tp_typequal & TQ_CONST))
		exp->ex_flags |= EX_READONLY;
	if ((sd->sd_type->tp_typequal & TQ_VOLATILE) || (tp->tp_typequal & TQ_VOLATILE))
...
void ch3cast(struct expr** expp, int oper, struct type* tp)
{
	...
	*/
	...
	switch (oper)
	{
		...
		case '=':
		case RETURN:
			ascompat = 1; /* assignment compatibility */
			/* fallthrough */
		case '-':
		case '<':
		case '>':
	...
	if (equal_type(tp, oldtp, qual_lev, 0))
	{
		...
		{
			if ((tp->tp_up->tp_typequal & oldtp->tp_up->tp_typequal) != oldtp->tp_up->tp_typequal)
			{
	...
	else if (tp->tp_fund == VOID)
	{
		/* easy again */
		exp->ex_type = void_type;
	}
	else if (is_arith_type(oldtp) && is_arith_type(tp))
	{
	...
	else if (oldtp->tp_fund == POINTER && tp->tp_fund == POINTER)
	{
		switch (oper)
		{
			case EQUAL:
			...
			case ':':
				...
				{
					if (tp->tp_up->tp_fund == VOID && oldtp->tp_up->tp_fund != FUNCTION)
					{
						break; /* switch */
					}
					if (oldtp->tp_up->tp_fund == VOID && tp->tp_up->tp_fund != FUNCTION)
					{
						break; /* switch */
					}
					if (oldtp->tp_up->tp_fund == VOID && is_cp_cst(exp) && exp->VL_VALUE == 0)
						break; /* switch */
				}
				/* falltrough */
			default:
				if (oper == CASTAB)
					...
					expr_strict(exp, "incompatible pointers in %s", symbol2str(oper));
				break;
			case CAST:
				break;
		}
	...
	else if (oldtp->tp_fund == POINTER && is_integral_type(tp))
	{
		/* from pointer to integral */
		if (oper != CAST)
			expr_strict(exp, "illegal conversion of pointer to %s", symbol2str(tp->tp_fund));
		if (oldtp->tp_size > tp->tp_size)
			expr_warning(
			    exp, "conversion of pointer to %s loses accuracy", symbol2str(tp->tp_fund));
		if (oldtp->tp_size != tp->tp_size)
		{
			int2int(expp, tp);
		}
	...
	else if (tp->tp_fund == POINTER && is_integral_type(oldtp))
	{
		...
		switch (oper)
		{
			...
			case RETURN:
				if (is_cp_cst(exp) && exp->VL_VALUE == 0)
					break;
			default:
				expr_strict(exp, "illegal conversion of %s to pointer", symbol2str(oldtp->tp_fund));
				break;
		}
		if (oldtp->tp_size > tp->tp_size)
			expr_warning(
			    exp, "conversion of %s to pointer loses accuracy", symbol2str(oldtp->tp_fund));
		if (oldtp->tp_size != tp->tp_size)
		{
			int2int(expp, tp);
		}
	...
	else if (oldtp->tp_fund == ERRONEOUS)
	{
		/* we just won't look */
		exp->ex_type = tp; /* brute force */
	}
	else if (oldtp->tp_size == tp->tp_size && oper == CAST)
	{
	...
	else
	{
		if (oldtp->tp_fund != ERRONEOUS && tp->tp_fund != ERRONEOUS)
			expr_error(
			    exp, "cannot convert %s to %s", symbol2str(oldtp->tp_fund),
...
int equal_type(struct type* tp, struct type* otp, int qual_lev, int diag)
{
	if (tp == otp)
		return 1;
	if (!tp || !otp || (tp->tp_fund != otp->tp_fund) || (tp->tp_unsigned != otp->tp_unsigned)
	    || (tp->tp_align != otp->tp_align))
		return 0;
	if (tp->tp_size != otp->tp_size)
	{
		if (tp->tp_fund != ARRAY || (tp->tp_size != -1 && otp->tp_size != -1))
			return 0;
	}
	...
	switch (tp->tp_fund)
	{
		...
		case ARRAY:
			...
			*/
			if (tp->tp_size != otp->tp_size && (tp->tp_size != -1 && otp->tp_size != -1))
				return 0;
			return equal_type(tp->tp_up, otp->tp_up, qual_lev /* ??? +1 */, diag);
		...
		if (!equal_type(pl->pl_type, opl->pl_type, -1, diag))
		{
			if (diag)
			{
				if (!(pl->pl_flag & PL_ERRGIVEN) && !(opl->pl_flag & PL_ERRGIVEN))
					error("incorrect type for parameter %s of definition", opl->pl_idf->id_text);
				pl->pl_flag |= PL_ERRGIVEN;
				opl->pl_flag |= PL_ERRGIVEN;
			}
			retval = 0;
	...
	if (prot->pl_flag & PL_ELLIPSIS)
	{
		if (prot->pl_flag & PL_ERRGIVEN)
		{
			if (pl)
				...
				error("ellipsis terminator in previous (prototype) declaration");
	...
	while (prot)
	{
		...
		if (fund == CHAR || fund == SHORT || fund == FLOAT)
		{
			if (diag && !(prot->pl_flag & PL_ERRGIVEN))
				error(
				    "illegal %s parameter in %sdeclaration", symbol2str(fund),
				    (opl ? "previous (prototype) " : ""));
			prot->pl_flag |= PL_ERRGIVEN;
			retval = 0;
		}
		prot = prot->next;
	...
	*/
	...
	else if (fund == STRUCT || fund == UNION)
	{
		if (recurqual(exp->ex_type, TQ_CONST))
			expr_error(exp, "operand of %s contains a const-qualified member", oper_string);
	}
	...
	{ /* turn e into e' where typeof(e') = typeof (f op e) */
...
#ifndef NOBITFIELD
	exp = new_oper(fund == FIELD ? exp->ex_type->tp_up : exp->ex_type, exp, oper, expr);
#else /* NOBITFIELD */
