	...
	switch (oper)
	{
		case '*': /* 3.3.3.2 */
			...
			if (expr->ex_type->tp_fund != POINTER)
			{
				...
				{
					expr_error(
					    expr, "* applied to non-pointer (%s)", symbol2str(expr->ex_type->tp_fund));
				}
			}
			else
			{
				if (is_ld_cst(expr))
					...
					*expp = new_oper(expr->ex_type->tp_up, NILEXPR, '*', expr);
				expr = *expp;
				expr->ex_lvalue
				    = (expr->ex_type->tp_fund != ARRAY && expr->ex_type->tp_fund != FUNCTION);
		...
		case ADDRESSOF:
			...
			else if ((*expp)->ex_type->tp_fund == FUNCTION)
			{
				(*expp)->ex_type = pointer_to((*expp)->ex_type, 0);
			}
			else
				...
				else
				{
					...
					if (ISNAME(*expp))
					{
						struct def* def = (*expp)->VL_IDF->id_def;
						...
						*/
						...
						{
							expr_error(*expp, "& on register variable not allowed");
							break; /* break case ADDRESSOF */
						}
					}
					(*expp)->ex_type = pointer_to((*expp)->ex_type, (*expp)->ex_type->tp_typequal);
					(*expp)->ex_lvalue = 0;
		...
		case '~':
		{
			...
			{
				expr_error(*expp, "~ not allowed on %s operands", symbol2str(fund));
				erroneous2int(expp);
				break;
			}
			/* FALLTHROUGH */
		...
		case SIZEOF:
			if (ISNAME(*expp) && (*expp)->VL_IDF->id_def->df_formal_array)
				expr_warning(
				    *expp, "sizeof formal array %s is sizeof pointer!", (*expp)->VL_IDF->id_text);
			expr = intexpr(
			    (*expp)->ex_class == String
			        ? (arith)((*expp)->SG_LEN)
			        : size_of_type((*expp)->ex_type, symbol2str((*expp)->ex_type->tp_fund)),
			    UNSIGNED);
