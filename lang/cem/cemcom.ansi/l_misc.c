...
check_hiding(idf, lvl, sc) struct idf* idf;
int lvl;
int sc;
{
	...
	*/
	...
	if (def && def->df_level < lvl
	    && !(lvl == L_FORMAL2 || def->df_level == L_UNIVERSAL || sc == GLOBAL || sc == EXTERN))
	{
		warning(
		    "%s is already defined as a %s", idf->id_text,
...
lint_new_oper(expr) struct expr* expr;
{
	...
	*/
	...
	int l_fund = left == 0 ? 0 : /* for monadics */
	    left->ex_type->tp_fund;
	int r_fund = right == 0 ? 0 : /* for ( without parameters */
	...
	*/
	switch (oper)
	{
		...
		case ORAB:
			...
			if (left->ex_class == Value && left->VL_CLASS == Const && left->VL_VALUE == 0)
			{
				return;
			}
	}
	...
	switch (oper)
	{
		...
		case EQUAL:
		case NOTEQUAL:
			if ((l_fund == ENUM || r_fund == ENUM) && left->ex_type != right->ex_type)
			{
...
static lint_enum_arith(l_fund, oper, r_fund) int l_fund, oper, r_fund;
{
	if (l_fund == ENUM && r_fund != CHAR && r_fund != SHORT && r_fund != INT)
	{
		warning("%s on enum and %s", symbol2str(oper), symbol2str(r_fund));
	}
	else if (r_fund == ENUM && l_fund != CHAR && l_fund != SHORT && l_fund != INT)
	{
		warning("%s on %s and enum", symbol2str(oper), symbol2str(l_fund));
	}
}

static lint_conversion(from_expr, to_fund) struct expr* from_expr;
int to_fund;
{
	...
	*/
	...
	{
		struct expr* bits = is_cp_cst(from_expr->OP_LEFT) ? from_expr->OP_LEFT
	...
	{
		awarning(
		    "conversion from %s to %s may lose accuracy", symbol2str(from_fund),
		    symbol2str(to_fund));
	}
}

static int numsize(fund)
{
	switch (fund)
	{
		case CHAR:
...
lint_ret_conv(from_expr) struct expr* from_expr;
{
	lint_conversion(from_expr, func_type->tp_fund);
}

lint_ptr_conv(from, to) short from, to;
{
	...
	{
		hwarning("pointer to char may not align correctly for a %s", symbol2str(to));
	}
	else
	{
		warning("pointer to %s may not align correctly for a %s", symbol2str(from), symbol2str(to));
	}
}

lint_relop(left, right, oper) struct expr* left, *right;
int oper; /* '<', '>', LESSEQ, GREATEREQ, EQUAL, NOTEQUAL */
{
	/* left operand may be converted */
	if (left->ex_class == Oper && left->OP_OPER == INT2INT)
	{
		left = left->OP_RIGHT;
	}

	/* <unsigned> <relop> <neg-const|0> is doubtful */
	if (left->ex_type->tp_unsigned && right->ex_class == Value && right->VL_CLASS == Const)
	{
	...
	if (left->ex_type->tp_fund == CHAR && right->ex_class == Value && right->VL_CLASS == Const
