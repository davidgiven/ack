...
static void check_selector(struct idf*, struct type*);
	...
	struct field *fd)
{
	...
	*sdefpp = &newsdef->sd_sdef;
	...
	else if (stp->tp_fund == UNION && fd == 0)
	{
		/*	Note: the case that a bitfield is declared is
		    handled by add_field() !
		*/
...
static void check_selector(struct idf* idf, struct type* stp)
{
...
void declare_struct(int fund, struct idf* idf, struct type** tpp)
{
	...
	*/
	...
	if (tg && tg->tg_type->tp_size < 0 && tg->tg_type->tp_fund == fund
	    && (tg->tg_level == level
	        || (level >= L_FORMAL2 && level <= L_LOCAL && tg->tg_level == L_FORMAL2)))
	{
		...
		if (tg->tg_busy)
		{
			error("recursive declaration of struct/union %s", idf->id_text);
	...
	else
	{
		...
		*tgp = tg;
		...
		tg->tg_type->tp_align = fund == ENUM ? int_align
...
void apply_struct(int fund, struct idf* idf, struct type** tpp)
{
	...
	*/
	...
	if (*tgp)
	{
		...
		{
			error(
			    "tag %s indicates a %s, not a %s", idf->id_text,
			    symbol2str((*tgp)->tg_type->tp_fund), symbol2str(fund));
		}
		*tpp = (*tgp)->tg_type;
	}
	else
    ...
    struct type* stp) /* current struct descriptor	*/
{
	...
	*/
	...
	if (fd->fd_width < 0 || (fd->fd_width == 0 && !is_anon_idf(idf)) || fd->fd_width > bits_in_type)
	{
	...
	switch ((*fdtpp)->tp_fund)
	{
		...
		case INT:
			...
			{
				error("bit field type %s does not fit in a word", symbol2str((*fdtpp)->tp_fund));
				*fdtpp = error_type;
				return field_offset;
			}
			break;
		...
		default:
			...
			error("field type cannot be %s", symbol2str((*fdtpp)->tp_fund));
			*fdtpp = error_type;
			return field_offset;
	}
	...
	*/
	if (fd->fd_width >= 8 * sizeof(arith))
		fd->fd_mask = -1;
	else
		fd->fd_mask = (1L << fd->fd_width) - 1;
