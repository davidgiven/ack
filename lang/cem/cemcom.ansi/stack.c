...
void unstack_level(void)
{
	...
	*/
	while (local_level->sl_entry)
	{
		...
		while ((tag = idf->id_tag) && tag->tg_level >= level)
		{
			...
			printf(
			    ...
			    (def->df_alloc == 0)                ? "no alloc"
			        ...
			        : (def->df_alloc == ALLOC_DONE) ? "alloc done"
			                                        : "illegal alloc info",
			    symbol2str(def->df_sc), def->df_initialized ? "init" : "no init",
			    def->df_used ? "used" : "not used");
		}
#endif /* DEBUG */
		if (def->df_sc == STATIC && def->df_type->tp_fund == FUNCTION && !def->df_initialized)
		{
			/* orphaned static function */
			warning("static function %s never defined, %s", idf->id_text, "changed to extern");
			def->df_sc = EXTERN;
		}

		if (def->df_alloc == ALLOC_SEEN && !def->df_initialized)
		{
