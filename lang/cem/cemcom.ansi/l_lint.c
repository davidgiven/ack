...
lint_expr(expr, used) struct expr* expr;
...
static struct expr_state* expr2state(expr, val, used)
struct expr* expr;
...
static struct expr_state* value2state(expr, val)
struct expr* expr;
int val; /* RVAL or LVAL */
{
...
static struct expr_state* oper2state(expr, val, used)
struct expr* expr;
int val; /* RVAL or LVAL */
int used; /* USED or IGNORED */
{
	...
	switch (oper)
	{
		...
		case '(':
			...
			if (ISNAME(left))
			{
				fill_outcall(expr, expr->ex_type->tp_fund == VOID ? VOIDED : used);
				outcall();
		...
		case OR:
		case AND:
			esp_l = expr2state(left, RVAL, oper == ',' ? IGNORED : USED);
			esp_r = expr2state(right, RVAL, oper == ',' ? used : USED);
			check_and_merge(expr, &esp_l, esp_r);
		...
		case Oper:
			...
			switch (oper)
			{
				...
				case '/':
					...
					*/
					if (expr->OP_TYPE->tp_fund == INT && expr->OP_LEFT->OP_OPER == '-'
...
static add_expr_state(value, to_state, espp) struct value value;
struct expr_state** espp;
{
	...
	/* try to find the esp */
	while (esp && !(esp->es_idf == value.vl_data.vl_idf && esp->es_offset == value.vl_value))
	{
		esp = esp->next;
	}
...
static referred_esp(esp) struct expr_state* esp;
{
...
static free_expr_states(esp) struct expr_state* esp;
{
	while (esp)
	{
...
struct expr_state* esp;
{
	...
	{
		printf(
		    " %s[%d]%c%c%c ", esp->es_idf->id_text, esp->es_offset, (esp->es_used ? 'U' : ' '),
		    (esp->es_referred ? 'R' : ' '), (esp->es_set ? 'S' : ' '));
		esp = esp->next;
	}
	printf(">\n");
}
