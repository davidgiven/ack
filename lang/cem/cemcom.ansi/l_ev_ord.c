...
check_and_merge(expr, espp, esp) struct expr* expr;
	...
	for (p1 = *espp; p1; p1 = p1->next)
	{
		...
		while (p2)
		{
			if (/* p1 and p2 refer to the same location */
			    p1->es_idf == p2->es_idf && p1->es_offset == p2->es_offset)
			{
				/* check */
				if (!is_sequencer)
...
static check_ev_order(esp1, esp2, expr) struct expr_state* esp1, *esp2;
struct expr* expr;
{
	if ((esp1->es_used && esp2->es_set) || (esp1->es_set && esp2->es_used)
	    || (esp1->es_set && esp2->es_set))
	{
		expr_warning(
		    expr, "result of %s depends on evaluation order on %s", symbol2str(expr->OP_OPER),
		    esp1->es_idf->id_text);
	}
}
