...
static struct lint_stack_entry* top_ls;

/* global variables for the brace stack */
static int brace_count;
static struct brace* top_br;

/* global variables for the function return */
static int valreturned; /* see l_lint.h */
static int return_warned;

static end_brace();
static lint_1_local();
static lint_1_global();
static start_loop_stmt();
static check_autos();
static struct auto_def* copy_auto_list();
static free_auto_list();
static struct state* copy_state();
static Free_state();
static remove_settings();
static struct auto_def* merge_autos();
static merge_states();
static struct lint_stack_entry *find_wdf(), *find_wdfc(), *find_cs();
static cont_merge();
static break_merge();
static struct lint_stack_entry* mk_lint_stack_entry();
static lint_push();
static lint_pop();
...
lint_end_local(stl) struct stack_level* stl;
{
...
static end_brace(stl) struct stack_level* stl;
{
...
static lint_1_local(idf, def) struct idf* idf;
struct def* def;
{
	int sc = def->df_sc;

	if ((sc == STATIC || sc == LABEL) && !def->df_used && !is_anon_idf(idf))
	{
		def_warning(
		    def, "%s %s not applied anywhere in function %s", symbol2str(sc), idf->id_text,
		    func_name);
	}

	if (loptions['h'] && sc == AUTO && !def->df_initialized && def->df_firstbrace != 0
	    && def->df_minlevel != level && !is_anon_idf(idf))
	{
		int diff = def->df_minlevel - level;

		def_warning(
		    def, "local %s could be declared %d level%s deeper", idf->id_text, diff,
		    (diff == 1 ? "" : "s"));
	}
}

lint_end_global(stl) struct stack_level* stl;
{
	struct stack_entry* se = stl->sl_entry;
...
static lint_1_global(idf, def) struct idf* idf;
struct def* def;
{
	int sc = def->df_sc;
	struct type* tp = def->df_type;
	int fund = tp->tp_fund;
		...
		case IMPLICIT:
			...
			if (sc == STATIC && !def->df_used)
			{
				if (def->df_set)
				{
					...
					{
						def_warning(
						    def, "%s %s %s set but not used", symbol2str(sc), symbol2str(fund),
						    idf->id_text);
					}
				}
				else
				{
					...
					{
						def_warning(
						    def, "%s %s %s not used anywhere", symbol2str(sc), symbol2str(fund),
						    idf->id_text);
					}
				}
			}
			if (loptions['x'])
			{
				...
				if ((sc == EXTERN || sc == GLOBAL) && def->df_alloc == 0 && !def->df_set
				    && !def->df_initialized && !def->df_used
				    && strcmp(&fn[strlen(fn) - 2], ".c") == 0 && !is_anon_idf(idf)
				    && fund != ERRONEOUS)
				{
					def_warning(
					    def, "%s %s %s not used anywhere", symbol2str(sc), symbol2str(fund),
...
change_state(idf, to_state) struct idf* idf;
	...
	switch (to_state)
	{
		...
		case USED:
			if (!a->ad_set)
			{
				...
				{
					warning(
					    "variable %s%s uninitialized", idf->id_text,
...
static check_autos()
{
	...
	while (a && a->ad_def->df_level == level)
	{
		...
		if (!def->df_used && !is_anon_idf(idf))
		{
			...
			{
				def_warning(def, "%s set but not used in function %s", idf->id_text, func_name);
			}
			else
			{
				def_warning(def, "%s not used anywhere in function %s", idf->id_text, func_name);
			}
		}
...
lint_end_formals()
{
	...
	while (se)
	{
		...
		if ((def && !def->df_used) && !(f_ARGSUSED || LINTLIB) && !is_anon_idf(se->se_idf))
		{
			def_warning(def, "argument %s not used in function %s", se->se_idf->id_text, func_name);
		}
		se = se->next;
	}
}

static struct auto_def* copy_auto_list(from_al, lvl)
...
static free_auto_list(a) struct auto_def* a;
{
	while (a)
	{
		struct auto_def* aux = a;
...
static struct state* copy_state(from_st, lvl)
...
static Free_state(stp) struct state** stp;
{
...
static remove_settings(st, lvl) struct state* st;
int lvl;
{
...
static merge_states(st1, st2, lvl, mode) struct state* st1, *st2;
int lvl;
int mode; /* NORMAL or CASE_BREAK */
{
	...
	if (st1->st_notreached)
	{
		...
		{
			st2->st_auto_list = merge_autos(st1->st_auto_list, st2->st_auto_list, lvl, USE_ONLY);
		}
	}
	else if (st2->st_notreached)
	{
		...
		st2->st_notreached = 0;
		st2->st_warned = 0;
		st2->st_auto_list = merge_autos(tmp, st2->st_auto_list, lvl, USE_ONLY);
		free_auto_list(tmp);
	}
	else
	{
		/* both st1 and st2 reached */
		st2->st_auto_list = merge_autos(st1->st_auto_list, st2->st_auto_list, lvl, mode);
	}
}

static struct auto_def* merge_autos(a1, a2, lvl, mode)
...
int mode;
{
	...
	while (a1)
	{
		...
		if (mode != USE_ONLY)
		{
			if ((!a2->ad_set && (a1->ad_set || a1->ad_maybe_set))
			    || (mode == NORMAL && !a1->ad_set && (a2->ad_set || a2->ad_maybe_set)))
			{
				a2->ad_set = 0;
				a2->ad_maybe_set = 1;
			}
...
static struct lint_stack_entry* find_wdfc()
{
	struct lint_stack_entry* lse = top_ls;
...
static struct lint_stack_entry* find_cs()
{
	struct lint_stack_entry* lse = top_ls;
...
start_else_part()
{
	...
	*/
	...
	top_ls->ls_current = copy_state(top_ls->ls_previous->ls_current, level);
...
end_if_else_stmt()
{
	...
	*/
	...
	merge_states(top_ls->LS_IF, top_ls->ls_current, top_ls->ls_level, NORMAL);
...
end_if_stmt()
{
	...
	*/
	...
	merge_states(top_ls->ls_current, top_ls->ls_previous->ls_current, top_ls->ls_level, NORMAL);
...
static start_loop_stmt(looptype, cst, cond)
{
	...
	{
		/* while (0) | for (;0;) */
		hwarning("condition in %s statement is always false", symbol2str(looptype));
...
end_switch_stmt()
{
	...
	if (!top_ls->LS_DEFAULT_MET)
	{
		...
		if (top_ls->LS_BREAK)
		{
			merge_states(top_ls->ls_current, top_ls->LS_BREAK, top_ls->ls_level, NORMAL);
			Free_state(&top_ls->ls_current);
		}
		else
		{
	...
	if (top_ls->LS_BREAK)
	{
		merge_states(top_ls->LS_CASE, top_ls->LS_BREAK, top_ls->ls_level, CASE_BREAK);
	...
	top_ls->LS_BREAK->st_notreached = top_ls->ls_previous->ls_current->st_notreached;
	/* yack */
	Free_state(&top_ls->ls_previous->ls_current);
...
lint_case_stmt(dflt)
{
	...
	if (cs_entry != top_ls)
	{
		warning("%s statement in strange context", dflt ? "default" : "case");
		return;
	}
	...
	switch (cs_entry->ls_class)
	{
		...
		case CASE:
			...
			merge_states(
			    top_ls->ls_current, top_ls->ls_previous->LS_CASE, top_ls->ls_previous->ls_level,
			    NORMAL);
			merge_states(
			    top_ls->ls_current, top_ls->ls_previous->ls_current, top_ls->ls_previous->ls_level,
			    NORMAL);
			Free_state(&top_ls->ls_current);
			top_ls->ls_current
			    = copy_state(top_ls->ls_previous->ls_current, top_ls->ls_previous->ls_level);
...
lint_break_stmt()
{
	...
	switch (lse->ls_class)
	{
		...
		case CASE:
			...
			merge_states(
			    lse->ls_current, lse->ls_previous->ls_current, lse->ls_previous->ls_level, NORMAL);
			if (lse->ls_previous->LS_BREAK)
			{
				merge_states(
				    top_ls->ls_current, lse->ls_previous->LS_BREAK, lse->ls_previous->ls_level,
				    NORMAL);
			}
			else
			{
				lse->ls_previous->LS_BREAK
				    = copy_state(top_ls->ls_current, lse->ls_previous->ls_level);
			}
			if (lse == top_ls)
			{
...
static cont_merge(lse) struct lint_stack_entry* lse;
{
	/* merge for continue statements */
	if (lse->LS_BODY)
	{
		merge_states(top_ls->ls_current, lse->LS_BODY, lse->ls_level, NORMAL);
	}
	else
	{
...
static break_merge(lse) struct lint_stack_entry* lse;
{
	/* merge for break statements */
	if (lse->LS_LOOP)
	{
		merge_states(top_ls->ls_current, lse->LS_LOOP, lse->ls_level, NORMAL);
	}
	else
	{
...
lint_return_stmt(e) int e;
{
	...
	if (valreturned == NORETURN)
	{
		...
		if (e == NOVALRETURNED && !func_notypegiven && fund != VOID && fund != ERRONEOUS)
		{
			warning(
			    "function %s declared %s%s but no value returned", func_name,
			    (func_type->tp_unsigned && fund != POINTER) ? "unsigned " : "", symbol2str(fund));
			/* adjust */
			e = VALRETURNED;
		}
		valreturned = e;
	}
	else if (valreturned != e && !return_warned)
	{
		warning("function %s does not always return a value", func_name);
		return_warned = 1;
	}
...
lint_jump_stmt(idf) struct idf* idf;
{
...
static struct lint_stack_entry* mk_lint_stack_entry(cl)
int cl;
{
...
static lint_push(lse) struct lint_stack_entry* lse;
{
...
static print_autos(a) struct auto_def* a;
{
	while (a)
	{
		struct idf* idf = a->ad_idf;
		...
		printf(
		    "(u%ds%dm%d U%dS%d) ", a->ad_used, a->ad_set, a->ad_maybe_set, def->df_used,
		    def->df_set);
		a = a->next;
	}
}

static pr_lint_state(nm, st) char* nm;
struct state* st;
{
	printf("%s: ", nm);
	if (st)
